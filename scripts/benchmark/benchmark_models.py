...
VARIANT_REGISTRY: List[ModelVariant] = [
    # ── LLM variants (quantization) ──────────────────────────────────────
    ModelVariant("llm", "Qwen 2.5 7B NVFP4", "nvfp4",
                 "qwen2.5-7b-nvfp4", True, 3200,
                 "FP4 Blackwell-native tensor cores"),
    ModelVariant("llm", "Qwen 2.5 7B AWQ", "awq",
                 "qwen2.5-7b-awq", True, 4500,
                 "INT4-AWQ (W4A16, group=128)"),
    ModelVariant("llm", "Qwen 2.5 7B FP16", "fp16",
                 "qwen2.5-7b-instruct", False, 15000,
                 "Full precision HuggingFace (baseline)"),
    # ── LLM variants (cross-model: different sizes) ──────────────────────
    ModelVariant("llm", "Qwen 2.5 3B AWQ", "awq",
                 "qwen2.5-3b-awq", True, 2000,
                 "INT4-AWQ 3B — balanced tier"),
    # ── LLM variants (cross-model: different architectures) ──────────────
    ModelVariant("llm", "Llama 3.1 8B AWQ", "awq",
                 "llama-3.1-8b-awq", True, 4800,
                 "INT4-AWQ Llama 3.1 (alternative architecture)"),
    # HF-only fallbacks (no TRT engine needed — uses transformers inferencer)
    ModelVariant("llm", "Qwen 2.5 3B HF", "fp16",
                 "qwen2.5-3b-instruct-awq", False, 2000,
                 "Qwen 3B via HuggingFace transformers"),
    ModelVariant("llm", "Llama 3.1 8B HF", "fp16",
                 "llama-3.1-8b-instruct-awq", False, 4800,
                 "Llama 3.1 via HuggingFace transformers"),

    # ── Conversation variants (unified STT+LLM+TTS) ─────────────────────
    ModelVariant("conversation", "Qwen2.5-Omni 7B NVFP4", "nvfp4",
                 "qwen2.5-7b-nvfp4", True, 3500,
                 "Unified STT+LLM, native audio I/O (default)"),
    ModelVariant("conversation", "Qwen2.5-Omni 3B AWQ", "awq",
                 "qwen2.5-3b-awq", True, 2000,
                 "Unified STT+LLM, 3B balanced tier"),
    ModelVariant("conversation", "Gemma 4 E4B", "fp16",
                 "gemma-4-e4b", False, 3000,
                 "Audio+vision input, text-only output (needs Kokoro TTS sidecar)"),
    ...
    # ── CV variants ──────────────────────────────────────────────────────
    ModelVariant("seg", "YOLOv8n-seg TRT", "fp16",
                 "yolov8n-seg.engine", True, 50,
                 "TensorRT FP16"),
    ModelVariant("seg", "YOLOv8s-seg TRT", "fp16",
                 "yolov8s-seg.engine", True, 100,
                 "YOLOv8s-seg (larger, more accurate)"),
]
...
def _run_standalone_eval(variant: ModelVariant) -> VariantResult:
    ...
    script_map = {
        "seg": os.path.join(PROJECT_ROOT, "scripts", "accuracy", "eval_segmentation.py"),
