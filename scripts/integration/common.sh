...
#
# Example:
#   ini_get config/omniedge.ini ports conversation_model 5572
...
# profile runtime scripts, mode test suites, and verification tools.
build_ld_library_path() {
    ...
    local parts=(
        ...
        # CUDA 12 compat libs — ORT 1.21 C++ SDK was built against CUDA 12
        # (libcublas.so.12, libcudart.so.12, libcufft.so.11). Installed via
        # pip `nvidia-*-cu12` packages in the Dockerfile, sitting alongside
        # the NGC CUDA 13 toolkit (different SONAME, no conflict).
...
# Args: $1 - repo ID (e.g. "google/gemma-4-E4B-it"), $2 - local directory, $3 - description
