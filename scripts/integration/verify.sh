...
if python3 -c "import cv2; assert cv2.cuda.getCudaEnabledDeviceCount() > 0" 2>/dev/null; then
    ...
    warn_fn "OpenCV CUDA modules not available — gpu_compositor disabled. Run: bash run_conversation.sh --install"
fi
...
if [ -f "$BUILD_DIR/CTestTestfile.cmake" ]; then
    ...
    warn_fn "Build directory not found — run 'bash run_conversation.sh --install' first"
fi
...
# ── Integration Tests ────────────────────────────────────────────────────────
# Description: Run mode-first integration tests and report overall pass/fail.
section "Integration Tests:"
if [ -x "$PROJECT_ROOT/test.sh" ]; then
    echo "  Running mode test suites (bash test.sh all)..."
    INTEG_RC=0
    bash "$PROJECT_ROOT/test.sh" all || INTEG_RC=$?
    if [ "$INTEG_RC" -eq 0 ]; then
        pass_fn "Mode test suites passed"
    else
        fail_fn "Mode test suites failed (exit $INTEG_RC)"
    fi
else
    warn_fn "Mode integration test runner not found (expected executable test.sh)"
fi
...
if [ "$FAIL" -gt 0 ]; then
    echo -e "  ${RED}Fix FAIL items. Run: bash run_conversation.sh --install${NC}"
    exit 1
fi
if [ "$WARN" -gt 0 ]; then
