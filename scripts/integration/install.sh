...
source "$SCRIPT_DIR/install_llm.sh"
...
run_test_mode() {
    ...
    section "Python Integration Tests"
    cd "$PROJECT_ROOT"
    if [ -f "scripts/integration/run_all.sh" ]; then
        echo "  Running scripts/integration/run_all.sh ..."
        bash scripts/integration/run_all.sh || record_error "Integration tests had failures"
    else
        warn "scripts/integration/run_all.sh not found — skipping"
    fi
