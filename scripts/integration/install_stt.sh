...
install_stt() {
    ...
    # Convert checkpoint (FP16 — SM120/Blackwell does not support INT8 weight-only GEMM)
    local CKPT_DIR="$OE_MODELS_DIR/trt_ckpt/whisper-turbo-fp16"
    if [ -d "$CKPT_DIR" ] && [ -n "$(ls -A "$CKPT_DIR" 2>/dev/null)" ]; then
        skip "Whisper FP16 checkpoint"
    else
        if [ ! -d "$TRTLLM_ROOT" ]; then
            ...
            if ! require_memory_gb 10 "Whisper FP16 checkpoint conversion"; then
                true
            else
                echo "  Converting Whisper checkpoint to FP16..."
    ...
    if [ -d "$CKPT_DIR" ] && [ -n "$(ls -A "$CKPT_DIR" 2>/dev/null)" ]; then
        local ENC_DIR="$OE_ENGINES_DIR/whisper/encoder_fp16"
        local DEC_DIR="$OE_ENGINES_DIR/whisper/decoder_fp16"
