...
# Description: Build a TRT-LLM engine (Llama, Phi, etc.)
...
if [[ "$INSTALL_CV" -eq 1 ]]; then
    ...
    # directly inside omniedge_bg_blur — no YOLOv8-seg TRT engine build
    # is required here.)
...
check_variant "Gemma-4 E4B (HF, default)"       "$OE_MODELS_DIR/gemma-4-e4b"
check_variant "Gemma-4 E2B (HF)"                 "$OE_MODELS_DIR/gemma-4-e2b"
...
echo -e "  ${BOLD}CV:${NC}"
check_variant "MediaPipe Selfie Seg ONNX"        "$OE_MODELS_DIR/bg_blur/mediapipe_selfie_seg.onnx"
check_variant "YOLOX-Nano ONNX (security)"       "$OE_MODELS_DIR/security/yolox_nano.onnx"
...
echo ""
echo "  # Or via profile launcher"
echo "  bash run_conversation.sh --model-test --quant-sweep --vram-budget 12288"
echo ""
exit $(( ERRORS > 0 ? 1 : 0 ))
