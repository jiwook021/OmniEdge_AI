...
" 2>/dev/null
    ...
    # facebook/sam2-hiera-tiny ships only .pt weights, so we pull the community
    # Apache-2.0 ONNX export from SharpAI/sam2-hiera-tiny-onnx (encoder.onnx +
    # decoder.onnx) and rename to the filenames onnx_sam2_inferencer expects.
    ...
    if [ -f "$SAM2_ENCODER" ] && [ -f "$SAM2_DECODER" ]; then
        ...
        mkdir -p "$SAM2_DIR"
        info "Downloading SAM2 Hiera Tiny ONNX (SharpAI/sam2-hiera-tiny-onnx)..."
        local _sam2_tmp
        _sam2_tmp="$(mktemp -d)"
        if hf download SharpAI/sam2-hiera-tiny-onnx \
                --include "encoder.onnx" "decoder.onnx" \
                --local-dir "$_sam2_tmp" >/dev/null 2>&1 \
           && [ -f "$_sam2_tmp/encoder.onnx" ] && [ -f "$_sam2_tmp/decoder.onnx" ]; then
            mv -f "$_sam2_tmp/encoder.onnx" "$SAM2_ENCODER"
            mv -f "$_sam2_tmp/decoder.onnx" "$SAM2_DECODER"
            rm -rf "$_sam2_tmp"
            ok "SAM2 Hiera Tiny ONNX installed: $SAM2_DIR"
        else
            rm -rf "$_sam2_tmp"
            warn "SAM2 ONNX download failed — segmentation will be disabled"
        fi
    fi
}
