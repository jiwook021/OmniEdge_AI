#!/usr/bin/env python3
"""OmniEdge_AI -- Multimodal Accuracy Benchmark (Gemma 4 E4B / Qwen2.5-Omni)
...
Modality tests:
  - Text-only QA: factual / reasoning (same as eval_llm_accuracy.py baseline)
...
Model loading fallback chain (first available wins):
  1. Qwen2.5-Omni-7B  (tokenizer: qwen2.5-7b-instruct-awq)
  2. Qwen2.5-Omni-3B  (tokenizer: qwen2.5-3b-instruct-awq)
  3. Gemma 4 E4B       (tokenizer: gemma-4-e4b)
  4. Exit 77 -- no multimodal model available
...
_MODEL_SEARCH_ORDER: List[Tuple[str, List[str], List[str]]] = [
    ("Qwen2.5-Omni-7B",
     ...
     ["qwen2.5-7b-instruct-awq", "qwen2.5-7b-instruct"]),
    ("Qwen2.5-Omni-3B",
     ["Qwen2.5-Omni-3B", "qwen2.5-omni-3b"],
     ["qwen2.5-3b-instruct-awq", "qwen2.5-3b-instruct"]),
    ("Gemma 4 E4B",
...
def _load_multimodal_inferencer():
    ...
    # Try Qwen2.5-Omni specific loader
    if "qwen" in model_name.lower():
        result = _try_load_qwen_omni(model_dir, tok_dir, model_name)
...
def _try_load_qwen_omni(model_dir: str, tok_dir: str, model_name: str):
    """Try loading Qwen2.5-Omni with its native multimodal API."""
    try:
        import torch
    ...
    try:
        # Qwen2.5-Omni uses AutoProcessor for multimodal input
        try:
        ...
        def _gen_vision(pil_image, prompt: str) -> Optional[str]:
            ...
            try:
                ...
                logger.debug("Qwen vision gen failed: %s", e)
                return None

        return _gen_text, _gen_audio, _gen_vision, f"{model_name} ({model_dir})"

    except Exception as e:
        logger.debug("Qwen Omni load failed: %s", e)
        return None, None, None, ""
...
def benchmark(warmup: int = 0) -> Dict[str, Any]:
    ...
    if gen_text is None:
        result["status"] = "skip"
        result["error"] = "No multimodal model available (need Qwen2.5-Omni or Gemma 4 E4B)"
        return result
...
def main() -> int:
    ...
    print("\033[1mMultimodal Accuracy Benchmark (Gemma 4 E4B / Qwen2.5-Omni)\033[0m\n")
