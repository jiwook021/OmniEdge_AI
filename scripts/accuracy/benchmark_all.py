...
def bench_llm(warmup: int = 1) -> BenchmarkResult:
    """Benchmark Qwen 2.5 7B LLM via eval_llm_accuracy."""
    r = _wrap_eval("llm", "qwen2.5_7b", eval_llm_accuracy.benchmark, warmup)

    inferencer_name = r.metrics.get("inferencer", "")
    if inferencer_name:
        r.model_name = f"qwen2.5_7b ({inferencer_name})"

    if r.latencies_ms and r.status != "skip":
        resp_lens = [r.metrics.get("mean_response_len", 10)] * len(r.latencies_ms)
        total_output_tokens = sum(max(1, int(rl) // 4) for rl in resp_lens if rl > 0)
        total_wall_s = sum(r.latencies_ms) / 1000.0
        if total_wall_s > 0 and total_output_tokens > 0:
            r.throughput = {
                "tokens_per_sec": round(total_output_tokens / total_wall_s, 1),
...
def bench_multimodal(warmup: int = 1) -> BenchmarkResult:
    """Benchmark Gemma 4 E4B / Qwen2.5-Omni multimodal via eval_multimodal_accuracy."""
...
def bench_seg(warmup: int = 1) -> BenchmarkResult:
    """Benchmark segmentation via eval_segmentation."""
    return _wrap_eval("seg", "yolov8n_seg", eval_segmentation.benchmark, warmup)
