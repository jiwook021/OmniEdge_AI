...
A multi-modal, real-time AI assistant for a single NVIDIA GPU, released as three mode-specific Docker images: **Conversation (with screen sharing)**, **Security**, and **Beauty**. OmniEdge_AI captures live video from a USB webcam and live audio from built-in microphones, accepts Windows desktop screen streams, and runs mode-specific C++ binaries behind a shared daemon + WebSocket bridge architecture.
...
- [Mode runtime behavior](#mode-runtime-behavior)
...
**Dynamic VRAM management.** Loading a model means `posix_spawn()`. Unloading means `SIGTERM` + `waitpid()`. A priority-based eviction scheduler (0 = evict first, 5 = evict last) decides what to kill when VRAM runs low. No module is unconditionally pinned; if the GPU cannot fit a model, the system degrades gracefully rather than crashing. Priorities shift on mode switches so active mode modules get highest priority while inactive modules become evictable. GPU tier auto-detection scales from 4 GB (minimal) to 16+ GB (ultra).

**Two conversation model variants.** Gemma-4 E4B (default, ~3 GB, audio+vision input with Kokoro TTS sidecar) and Gemma-4 E2B (lighter, ~2.5 GB, same capability profile). Users switch variants at runtime via the browser UI. On GPUs with less than 3 GB conversation headroom the system falls back to Gemma-4 E2B.
...
**Default video pipeline.** Live 1080p webcam feed with GPU-accelerated background blur (MediaPipe Selfie Segmentation via ONNX Runtime CUDA), ISP adjustments (brightness, contrast, saturation, sharpness), shape overlays, face recognition (SCRFD + AuraFace-v1), and nvJPEG encoding, all running at 30 fps. In released images, this is used primarily by Conversation and Beauty, while Security runs its own detection/recording path.
...
The same codebase is shipped as three separate runtime images/binaries for predictable VRAM behavior and simpler UI mode switching:

| Mode | Docker profile / service | Entry script | Frontend | Primary runtime binaries |
|:---|:---|:---|:---|:---|
| Conversation | `conversation` / `omniedge` | `run_conversation.sh` | `/frontend/conversation/index.html` | `omniedge_conversation`, `omniedge_tts`, `omniedge_bg_blur`, ingest nodes |
| Security | `security` / `omniedge-security` | `run_security_mode.sh` | `/frontend/security/index.html` | `omniedge_security_camera`, `omniedge_security_vlm`, ingest nodes |
| Beauty | `beauty` / `omniedge-beauty` | `run_beautymode.sh` | `/frontend/beauty/index.html` | `omniedge_beauty`, ingest nodes |

`run_all.sh` is deprecated and retained only as a compatibility shim.

---

## Architecture overview

Each release image runs one mode-specific binary set, but all modes share the same layered architecture:
...
graph TB
    ...
    subgraph GPU["GPU layer (NVIDIA, 4-16+ GB)"]
        AR["Always resident: BackgroundBlur (MediaPipe Selfie Seg) + ISP (~0.25 GB)\n+ driver reserve (~1.5 GB)"]
...
Four independent data planes feed into the system. Which nodes consume each plane depends on the active release mode (see [Mode runtime behavior](#mode-runtime-behavior)).

### Video pipeline

A USB camera frame enters the system through VideoIngestNode and fans out to whichever CV nodes the active mode spawns:

```mermaid
graph TD
    CAM["USB Camera"] --> VIN["VideoIngestNode\n(GStreamer capture)"]
    VIN -->|"BGR24 write"| SHM_V["/oe.vid.ingest SHM\n(CircularBuffer, 4 slots)"]
    VIN -->|"ZMQ PUB :5555\nvideo_frame"| NOTE["Mode-dependent consumers\n(see Mode runtime behavior)"]

    SHM_V --> PROFILE["Active mode CV nodes\nread latest slot"]
...
graph LR
    ...
    T["Text (ZMQ)"] --> CN
    CN --> MODEL["Gemma-4 E4B\nor E2B"]
    MODEL --> TXT["text tokens -> ZMQ 'llm_response'"]
    MODEL --> AOUT["text tokens -> Kokoro TTS\n-> ZMQ 'audio_output'"]
...
Production packaging is release-mode-first: **Conversation**, **Security**, and **Beauty**. Only one mode is active per running container. The daemon updates module priorities from `config/omniedge.ini` (`[profile_conversation]`, `[profile_security]`, `[profile_beauty]`), evicts modules that drop to priority `0`, waits for VRAM release, then spawns the target mode modules.

> **Internal compatibility note:** source code still contains legacy mode identifiers (for example `vision_model` and `sam2_segmentation`) used by lower-level orchestration and test surfaces. These are internal compatibility paths, not release modes.

### Default shared processes

These binaries are shared across release modes:

| Binary | Role | ZMQ PUB port |
|:---|:---|---:|
| `omniedge_daemon` | Process lifecycle, FSM, mode orchestration | 5571 |
| `omniedge_ws_bridge` | WebSocket server + ZMQ relay | 5570 |
| `omniedge_video_ingest` | Webcam capture via GStreamer | 5555 |
| `omniedge_audio_ingest` | Microphone capture + VAD feed | 5556 |
| `omniedge_screen_ingest` | Desktop capture ingress from Windows DXGI agent | 5577 |

### Release mode matrix

| Mode | Primary modules | Primary outputs | Typical VRAM |
|:---|:---|:---|---:|
| Conversation | `omniedge_conversation`, `omniedge_tts`, `omniedge_bg_blur` | `/video`, `/audio`, `/screen_video` | ~3.0-3.6 GB (Gemma variant dependent) |
| Security | `omniedge_security_camera`, `omniedge_security_vlm` (on-demand), `omniedge_bg_blur` | `/security_video`, security events/recordings | ~0.25 GB base (+ on-demand VLM) |
| Beauty | `omniedge_beauty`, `omniedge_bg_blur` | `/beauty_video` | ~0.25 GB |

### Conversation mode

Conversation mode runs the unified Gemma conversation stack with camera/screen/audio input and optional Kokoro TTS sidecar output. Users can switch between Gemma E2B and E4B from the UI without leaving conversation mode.

Active SHM (common path): `/oe.vid.ingest`, `/oe.aud.ingest`, `/oe.screen.ingest`, `/oe.cv.blur.jpeg`, `/oe.aud.tts`.

### Security mode

Security mode runs continuous detection + recording with security-specific events, annotated video output, and optional on-demand VLM analysis for clips.

Active SHM (common path): `/oe.vid.ingest`, `/oe.cv.security.jpeg`.

### Beauty mode

Beauty mode runs face beautification on live ingest and streams beautified frames through the dedicated beauty channel.

Active SHM (common path): `/oe.vid.ingest`, `/oe.cv.beauty.jpeg`.

### Mode switching

When mode changes (for example Conversation -> Security):

1. Daemon applies the target mode priority map.
2. Modules that become `priority=0` are terminated in dependency-safe order.
3. `VramGate` waits for driver-level VRAM reclamation.
4. Target-mode modules are spawned and health-checked.

This keeps mode switches deterministic and avoids carrying inactive model VRAM.
...
|:---|:---|:---:|:---|:---|:---|:---|
| `/oe.vid.ingest` | ShmCircularBuffer | 4 | VideoIngestNode | Active mode CV nodes + ConversationNode | BGR24 1920x1080 | 6.2 MB |
...
| 5563 | STTNode | `transcription` | No | Standalone Whisper path (alt to ConversationModel on 5572) |
...
| 5572 | ConversationModel | `transcription`, `llm_response`, `audio_output` | No | Unified model output (alt to standalone STTNode on 5563) |
...
When the user switches modes (for example, Conversation to Security):

1. Daemon calls `PriorityScheduler::applyProfile(newProfile, priorityMap)`.
2. All module priorities update according to the new mode map (loaded from `config/omniedge.ini` mode profile sections).
...
5. New mode modules are spawned via `VramGate::acquireVramForModule()`.

Conversation and Beauty typically keep `background_blur` loaded. Security mode keeps `background_blur` plus security-specific modules and evicts conversation-only modules.

### Graph-based orchestration (shadow mode)

A graph-based orchestrator (`core::pipeline_orchestrator::PipelineOrchestrator`) models the pipeline as a directed acyclic graph where vertices are OS processes and edges are IPC connections (SHM, ZMQ, WebSocket). Mode switches are computed as graph diffs:
...
`PipelineOrchestrator` is the canonical mode switcher. A `get_graph` ZMQ command returns the live pipeline graph as JSON for debugging.
...
Vanilla JavaScript SPA served by `omniedge_ws_bridge` on port 9001. Dark-theme conversational interface.

### WebSocket channels

| Channel | Data | Direction |
|:---|:---|:---|
| `/video` | Binary JPEG from BackgroundBlur SHM (30 fps, conversation/beauty flows) | Server to client |
...
```bash
bash scripts/install_models.sh                  # Downloads to ~/omniedge_models/
```
...
```bash
docker compose --profile conversation build      # conversation image
docker compose --profile security build          # security image
docker compose --profile beauty build            # beauty image
```

### 3. Launch

```bash
docker compose --profile conversation up -d      # http://localhost:9001
docker compose --profile security up -d          # http://localhost:9002
docker compose --profile beauty up -d            # http://localhost:9003
docker compose logs -f omniedge                  # or omniedge-security / omniedge-beauty
```

Each mode runs its own container/image and mode-specific frontend/binary set.
The entrypoint validates GPU access, checks model mounts, builds missing TRT engines, then starts mode-specific binaries.
...
# Inside container:
bash run_conversation.sh                         # Build + launch (conversation)
bash run_security_mode.sh                        # Build + launch (security)
bash run_beautymode.sh                           # Build + launch (beauty)
bash test.sh conversation                        # Profile tests
```

`run_all.sh` remains available only as a deprecated compatibility shim that forwards utility flags to `run_conversation.sh`. Use `run_*` scripts for all runtime launches.

### Other commands

| Command | Context | Purpose |
|:---|:---|:---|
| `docker compose --profile conversation up -d` | Host | Start conversation service |
| `docker compose --profile security up -d` | Host | Start security service |
| `docker compose --profile beauty up -d` | Host | Start beauty service |
| `docker compose logs -f omniedge-security` | Host | Stream security service logs |
| `bash run_conversation.sh --install` | Dev container | Install/build/launch conversation profile |
| `bash run_security_mode.sh --install` | Dev container | Install/build/launch security profile |
| `bash run_beautymode.sh --install` | Dev container | Install/build/launch beauty profile |
| `bash test.sh <conversation|security|beauty|all>` | Dev container | Run profile test suites |
...
Primary configuration file. Defines module launch order, per-module VRAM budgets, conversation model selection, TTS model, and multi-tier GPU tiers (minimal / balanced / standard / ultra). Uses `/opt/omniedge/` paths for Docker deployment.

### `config/omniedge.ini`

INI-format configuration for ZMQ port assignments, module enable/disable flags, mode priority maps (`[profile_conversation]`, `[profile_security]`, `[profile_beauty]`), inference settings, pipeline parameters, and logging levels. Installed to `/opt/omniedge/etc/omniedge.ini` by CMake.

### GPU tier system

Auto-detected via `GpuProfiler::probe()` (NVML). Override with `--profile <tier>`.

| Tier | VRAM | Default conversation model | Available release modes |
|:---|:---|:---|:---|
| Minimal | 4 GB | Gemma-4 E2B | Conversation |
| Balanced | 8 GB | Gemma-4 E4B | Conversation, Beauty |
| Standard | 12 GB | Gemma-4 E4B (both variants available) | Conversation, Security, Beauty |
| Ultra | 16+ GB | Gemma-4 E4B (expanded context) | Conversation, Security, Beauty (with larger headroom) |
...
# Direct registry runner (same suites via mode tags)
bash tests/run_tests.sh --mode conversation
bash tests/run_tests.sh --mode security
bash tests/run_tests.sh --mode beauty
```

`tests/run_tests.sh` auto-detects environment limits and marks non-runnable suites as skipped:

- GPU-tagged tests are skipped when CUDA runtime init fails (for example, driver/runtime mismatch).
- SHM-tagged tests are skipped when required `/dev/shm/oe.*` segments are not writable.

Force GPU-tagged tests anyway:

```bash
OE_REQUIRE_GPU_TESTS=1 bash tests/run_tests.sh --mode conversation
```
...
bash run_conversation.sh --model-test
```
...
| Metric | Target |
|:---|:---|
| PTT release to first audio (Gemma-4 + Kokoro) | < 1,300 ms |
...
`tools/mcp_server/` is a Python stdio MCP (Model Context Protocol) server that exposes the daemon's perception stack to Claude Desktop and Claude Code as first-class tools: `describe_screen`, `who_is_in_frame`, `record_clip`, `segment_object`, `switch_mode`, and others. The MCP server is not a daemon-managed node; Claude spawns it over stdio and it acts as a ZMQ client of the running daemon. If the daemon is offline the server still starts so tool schemas stay discoverable, but every call fails fast with `OE-MCP-1002`.

See [`tools/mcp_server/README.md`](tools/mcp_server/README.md) for install, config, and tool reference.

### Docker setup

The quickstart above covers the common case. For the full Docker guide — host prerequisites, GPU passthrough on WSL2 vs native Linux, model volume layout, rebuild triggers, and common failure modes — see [`DOCKER_SETUP.md`](DOCKER_SETUP.md).

### Claude Code skills

When working on the codebase with Claude Code, module-scoped work is routed through skills under `.claude/skills/` (one per module: `omniedge-daemon`, `omniedge-cv`, `omniedge-conversation`, `omniedge-ingest`, `omniedge-tts`, `omniedge-stt`, `omniedge-bridge`, `omniedge-security`, `omniedge-networking`, plus cross-cutting ones for build/run, CMake, code review, and architecture docs). The routing table and conventions live in [`.claude/CLAUDE.md`](.claude/CLAUDE.md).

## License

OmniEdge_AI is commercial software. Copyright © 2026 Jiwook Kim. All rights reserved. Use of OmniEdge_AI is governed by the end-user license agreement in [`EULA.md`](EULA.md); the top-level commercial license terms are in [`LICENSE`](LICENSE). Third-party components retain their own licenses — see the next section.

> The current [`LICENSE`](LICENSE) and [`EULA.md`](EULA.md) are drafts marked `DRAFT — ATTORNEY REVIEW REQUIRED` and must be finalized by counsel before public release.

For commercial licensing, redistribution, OEM bundling, or enterprise site licenses, contact **jiwook021@gmail.com**.

## Third-Party Components

OmniEdge_AI bundles, depends on, or interoperates with the following third-party software and AI model weights. A complete inventory with source URLs and notes appears in [`THIRD_PARTY_NOTICES.md`](THIRD_PARTY_NOTICES.md); required attribution text is in [`NOTICE`](NOTICE); full license texts are in the [`licenses/`](licenses/) directory.

| Component | Role | License | License file |
|---|---|---|---|
| [Gemma-4 E2B / E4B](https://ai.google.dev/gemma) | Conversation LLM | **Gemma Terms of Use** (Google LLC) | [`licenses/GEMMA_TERMS.txt`](licenses/GEMMA_TERMS.txt) |
| [Whisper Large V3 Turbo](https://github.com/openai/whisper) | Speech-to-text | MIT (OpenAI) | [`licenses/WHISPER_LICENSE.txt`](licenses/WHISPER_LICENSE.txt) |
| [Kokoro-82M](https://huggingface.co/hexgrad/Kokoro-82M) | Text-to-speech | Apache 2.0 | [`licenses/KOKORO_LICENSE.txt`](licenses/KOKORO_LICENSE.txt) |
| [YOLOX-Nano](https://github.com/Megvii-BaseDetection/YOLOX) | Object detection (Security mode) | Apache 2.0 (Megvii Inc.) | [`licenses/YOLOX_LICENSE.txt`](licenses/YOLOX_LICENSE.txt) |
| [AuraFace v1 / glintr100](https://huggingface.co/fal/AuraFace-v1) | Face recognition | Apache 2.0 (fal.ai) | [`licenses/AURAFACE_LICENSE.txt`](licenses/AURAFACE_LICENSE.txt) |
| [Segment Anything 2](https://github.com/facebookresearch/segment-anything-2) | Interactive segmentation | Apache 2.0 (Meta Platforms, Inc.) | [`licenses/SAM2_LICENSE.txt`](licenses/SAM2_LICENSE.txt) |
| [MediaPipe Selfie Segmentation](https://github.com/google-ai-edge/mediapipe) | Background blur | Apache 2.0 (Google LLC) | [`licenses/MEDIAPIPE_SEG_LICENSE.txt`](licenses/MEDIAPIPE_SEG_LICENSE.txt) |
| [TensorRT-LLM](https://github.com/NVIDIA/TensorRT-LLM) | LLM inference framework | Apache 2.0 (NVIDIA) | [`licenses/TENSORRT_LLM_LICENSE.txt`](licenses/TENSORRT_LLM_LICENSE.txt) |
| NGC TensorRT-LLM container + CUDA/cuDNN/NCCL/TensorRT runtime | Runtime base image | NVIDIA Deep Learning Container License | [`licenses/NVIDIA_DLC_LICENSE.txt`](licenses/NVIDIA_DLC_LICENSE.txt) |
| Docker Desktop | Runtime dependency (Windows/macOS) | Docker Subscription Service Agreement | [`licenses/DOCKER_DESKTOP_NOTICE.txt`](licenses/DOCKER_DESKTOP_NOTICE.txt) |

This product uses Gemma, subject to the [Gemma Prohibited Use Policy](https://ai.google.dev/gemma/prohibited_use_policy).

Docker Desktop is separately licensed by Docker, Inc. Docker Desktop is free for personal use and small businesses (organizations with fewer than 250 employees **and** less than US$10M in annual revenue); larger organizations must obtain a paid Docker subscription. OmniEdge_AI does not grant, sub-license, or cover a Docker Desktop license.
