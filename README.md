...
> **Version:** 2.3 · **Status:** Reference Architecture  
...
OmniEdge_AI is a **multi-modal, real-time AI assistant** that executes up to ten concurrent AI models on a single NVIDIA GPU (12 GB VRAM) under WSL2. It captures live 1080p video and 16 kHz audio from a USB webcam, processes them through inference pipelines (LLM, STT, TTS, VLM, face recognition, instance segmentation) with on-demand video denoising (BasicVSR++) and audio denoising (DTLN), and delivers a browser-based conversational interface with streaming text, synthesized speech, and augmented video.

### Architectural Invariants

| Invariant | Mechanism | Consequence |
|:---|:---|:---|
| **Process-per-model fault isolation** | 12 independent C++20 binaries, each with its own `cudaSetDevice(0)` context. No shared CUDA contexts, no MPS. | A `SIGSEGV` in the face recognition process cannot corrupt the LLM's KV cache. The CUDA driver reclaims the crashed process's VRAM as contiguous blocks. |
...
│  └────────────────┘                                                       │
│                                                                            │
│  Enhancement Models (On-Demand — not counted in default 12 GB budget)      │
│  ┌────────────────┐   ┌────────────────┐                                  │
│  │ BasicVSR++     │   │ DTLN           │                                  │
│  │ FP16 · 1.50 GB │   │ FP32 · 0.05 GB │                                  │
│  │ (on-demand)    │   │ (on-demand)    │                                  │
│  └────────────────┘   └────────────────┘                                  │
└──────────┬─────────────────────────────────────────────────────────────────┘
           │ POSIX SHM (data) + ZeroMQ PUB/SUB (control)
┌──────────┴─────────────────────────────────────────────────────────────────┐
│  12 Independent C++20 Processes (WSL2)                                     │
│                                                                            │
...
│  oe_video_denoise (BasicVSR++)   oe_audio_denoise (DTLN)                  │
└──────────┬─────────────────────────────────────────────────────────────────┘
           │ 5 WebSocket channels on :9001 (/video, /audio, /chat, /denoise_video, /denoise_audio)
┌──────────┴─────────────────────────────────────────────────────────────────┐
...
| `oe_daemon` | — | — | 0 | 5571 |
| `oe_video_denoise` | BasicVSR++ FP16 | ONNX Runtime CUDA EP | 1.50 GB | 5568 |
| `oe_audio_denoise` | DTLN FP32 | ONNX Runtime CUDA EP | 0.05 GB | 5569 |

> **Note:** `oe_video_denoise` and `oe_audio_denoise` are **on-demand** enhancement modules. They are not loaded at boot and do not count toward the default 12 GB VRAM budget. The daemon spawns them when the user toggles denoising from the web UI.
...
No module links to another's library or calls its functions. All inter-process communication flows through these two channels. Each module's ZMQ networking is consolidated into a single `MessageRouter` instance (see Section 6.4).
...
|:---|:---|
| `LLMBackend` | `TensorRTLLMBackend` (`createTensorRTLLMBackend()`) |
| `STTBackend` | `TrtWhisperBackend` (`createTrtWhisperBackend()`) |
| `TTSBackend` | `OnnxKokoroBackend` (`createOnnxKokoroBackend()`) |
| `VLMBackend` | `HybridMoondreamBackend` (`createHybridMoondreamBackend()`) |
| `BlurBackend` | GPU compositor pipeline (YoloSegEngine + ISP + nvJPEG) |
| `FaceRecogBackend` | `InspireFaceWrapper` (C API wrapper) |
| `DenoiseBackend` | `OnnxBasicVsrppBackend` (`createOnnxBasicVsrppBackend()`) |
| `AudioDenoiseBackend` | `OnnxDtlnBackend` (`createOnnxDtlnBackend()`) |

The active backend is selected from `omniedge_config.yaml`. Swapping Qwen for Llama 3 requires only a new `LLMBackend` implementation and a YAML config change — no other modules are affected.
...
| **Compile-time Tuning** | Per-module `*_constants.hpp` headers | Buffer sizes, ZMQ HWM, frame dimensions — requires rebuild |
| **Platform Detection** | `modules/common/include/platform_detect.hpp` | WSL2 detection (`OE_PLATFORM_WSL2`), Blackwell feature gates (`OE_HAS_BLACKWELL_FEATURES`) |
| **Runtime Defaults** | `modules/common/include/runtime_defaults.hpp` | Fallback values when YAML fields are missing (lifecycle, LLM, STT, CV defaults) |

Module `.cpp` files never contain bare numeric literals for tuning parameters.

### 3.5 WSL2 Constraint: `cudaHostAlloc` Only

`cudaHostRegister` is **unsupported** under WDDM (WSL2). All pinned host memory uses `cudaHostAlloc` instead. This is detected at compile time via `OE_PLATFORM_WSL2` in `modules/common/include/platform_detect.hpp`. The macro `OE_HOST_REGISTER_SUPPORTED` evaluates to 0 on WSL2 and 1 on native Linux.
...
| `oe_video_denoise` | 2 streams | `stream_copy`: frame H→D. `stream_infer`: BasicVSR++ ONNX inference + nvJPEG encode. |
| `oe_audio_denoise` | 1 stream | ONNX Runtime CUDA EP manages internally. Audio chunks are small (~2 KB), copy overhead negligible. |

**Stream priority** is defined per module in `modules/gpu/include/gpu/cuda_priority.hpp` (kLlm=-5, kStt=-5, kTts=-3, kVlm=-1, kCv=0, kVideoDenoise=-1, kAudioDenoise=0) but affects only **intra-process** scheduling — the relative priority of a module's own streams (e.g., copy vs. inference). Under the process-per-model isolation model (no MPS, see Section 4.1), each module runs in its own CUDA context. Stream priorities have **no effect** on cross-process GPU scheduling, which is managed by the WDDM driver's time-slicing scheduler. The voice pipeline's latency advantage comes from its sequential data dependency (STT output → LLM input → TTS input), not from GPU-level preemption.
...
|:---|:---|:---|
| `CMAKE_CUDA_ARCHITECTURES` | `75;80;86;89;90;100;120` | Multi-arch: Turing (75) through Blackwell (100, 120). Set in root `CMakeLists.txt`. TensorRT engines are still SM-specific — built per target GPU at deploy time. |
...
set(CMAKE_CUDA_ARCHITECTURES "75;80;86;89;90;100;120"
    CACHE STRING "CUDA architectures: Turing(75) → Blackwell(100,120)")
```

This compiles CUDA device code for all supported architectures from Turing through Blackwell. TensorRT engines remain non-portable — they must be rebuilt for each specific GPU via `trtexec` at deploy time.
...
Webcam (USB) ──→ GStreamer ──→ Video Ingest ──shm──→ ┬─→ Background Blur ──shm──→ WS Bridge ──→ Browser Canvas
                                                      ...
                                                      └─→ Video Denoise (on-demand) ──shm──→ WS Bridge ──→ Browser Canvas (split-screen)

Microphone ──→ GStreamer ──→ Audio Ingest ──shm──→ ┬─→ STT ──zmq──→ Daemon ──zmq──→ LLM ──zmq──→ TTS ──shm──→ WS Bridge ──→ Browser Audio
                                                    └─→ Audio Denoise (on-demand) ──shm──→ WS Bridge ──→ Browser Audio (comparison)
```
...
### 5.2 Video Ingest (`oe_video_ingest`)
...
### 5.3 Audio Ingest (`oe_audio_ingest`)
...
### 5.4 Background Blur (`oe_bg_blur`)
...
### 5.5 Face Recognition (`oe_face_recog`)
...
### 5.9 Moondream2 VLM (`oe_vlm`)

Moondream2 (1.86B params) provides image captioning triggered by `describe_scene`, `describe_uploaded_image`, or periodic timer.

**Hybrid architecture:** The text decoder uses custom cross-attention ops that cannot be cleanly exported to ONNX. Solution:
- **Python subprocess** (`vision_subprocess.py`) handles vision encoding using PyTorch
- **C++ process** reads pre-computed vision features and runs the text decoder via ONNX Runtime

**Image upload:** The browser frontend accepts JPEG/PNG file uploads (max 10 MiB). The binary image is sent on the `/chat` WebSocket using a framed binary protocol (`[4B json_len LE][JSON header][image bytes]`). The bridge writes the image to a temp file in `/tmp/oe_vlm_upload_*` and publishes `describe_uploaded_image` with the `image_path`. The VLM subprocess reads the file directly via `--image-file` instead of reading from video SHM, then follows the same hybrid encoding + decoding pipeline.
...
### 5.10 Enhancement Modules (On-Demand)

Two on-demand enhancement modules provide temporal video denoising and real-time audio denoising. Both are spawned by the daemon when the user toggles denoising from the web UI, and are terminated when denoising is disabled. Neither is included in the default 12 GB VRAM budget — they require additional headroom or eviction of lower-priority modules.

#### VideoDenoiseNode (`oe_video_denoise`)

Reads raw BGR frames from `/oe.vid.ingest` SHM, applies **BasicVSR++ temporal video denoising** over a sliding window of 5 frames, and writes denoised JPEG output to `/oe.vid.denoise` SHM.

- **Model:** BasicVSR++ FP16, ~1.50 GB VRAM
- **Inference:** ONNX Runtime with CUDA EP
- **SHM input:** `/oe.vid.ingest` (reads latest BGR frame, maintains internal 5-frame ring buffer)
- **SHM output:** `/oe.vid.denoise` (JPEG double-buffer, same protocol as `/oe.cv.blur.jpeg`)
- **ZMQ:** Publishes `denoised_frame` on port 5568 (`{shm:"/oe.vid.denoise", size:N}`)
- **Eviction priority:** 0 (lowest — evicted first, before Background Blur)
- **CUDA streams:** 2 streams — `stream_copy`: frame H→D, `stream_infer`: BasicVSR++ inference + nvJPEG encode

#### AudioDenoiseNode (`oe_audio_denoise`)

Reads raw PCM audio from `/oe.aud.ingest` SHM, applies **DTLN two-stage audio denoising** (separation network + signal reconstruction network), and writes cleaned PCM to `/oe.aud.denoise` SHM.

- **Model:** DTLN FP32 (two ONNX models: separation + reconstruction), ~50 MiB VRAM
- **Inference:** ONNX Runtime with CUDA EP
- **SHM input:** `/oe.aud.ingest` (PCM F32 16 kHz)
- **SHM output:** `/oe.aud.denoise` (PCM F32 16 kHz, double-buffer)
- **ZMQ:** Publishes `denoised_audio` on port 5569 (`{shm:"/oe.aud.denoise", samples:N}`)
- **Eviction priority:** 0 (lowest — evicted first, before Background Blur)
- **Stateful:** Maintains LSTM hidden state across chunks for temporal continuity

Both modules follow the standard OmniEdge process lifecycle: `posix_spawn()` to load, `SIGTERM` to unload, `module_ready` on startup, ZMTP heartbeat monitoring by the daemon.

### 5.11 WebSocket Bridge (`oe_ws_bridge`)
...
| Denoised Video | `/denoise_video` | Binary JPEG (denoised) | Server → Client |
| Denoised Audio | `/denoise_audio` | Binary PCM float32 (denoised) | Server → Client |
...
|:---|:---|:---|:---|:---|
| `/oe.vid.ingest` | VideoIngest | BgBlur, FaceRecog, VLM, VideoDenoise | 1920×1080 BGR (6.2 MB) | Double-buffer (`ShmVideoControl`, 64-byte header) |
| `/oe.aud.ingest` | AudioIngest | WhisperSTT, AudioDenoise | PCM F32 16 kHz | Ring buffer (4 slots, `ShmAudioHeader` 24-byte header) |
| `/oe.cv.blur.jpeg` | BgBlur | WS Bridge | JPEG (~50–200 KB, max 1 MiB/slot) | Double-buffer (`ShmJpegControl`, lock-free atomics) |
| `/oe.aud.tts` | KokoroTTS | WS Bridge | PCM F32 24 kHz (max 240,000 samples = 10 s) | Double-buffer (`ShmTtsHeader`, 24-byte header) |
| `/oe.vlm.features` | MoondreamVLM (Python subprocess) | MoondreamVLM (C++ decoder) | FP16 feature tensor (729 patches × 2048 dim, ~3 MB) | Single-buffer (`ShmVlmFeaturesHeader`, 32-byte header) |
| `/oe.vid.denoise` | VideoDenoise | WS Bridge | JPEG (~50–200 KB, max 1 MiB/slot) | Double-buffer (`ShmJpegControl`) |
| `/oe.aud.denoise` | AudioDenoise | WS Bridge | PCM F32 16 kHz (max 32,000 samples = 2 s) | Double-buffer (`ShmAudioHeader`) |
...
### 6.4 MessageRouter — Consolidated Networking

Every module binary uses a single `MessageRouter` instance (defined in `modules/zmq/include/zmq_transport/message_router.hpp`) that consolidates all ZMQ networking into one class:

- **ZMQ context** — one per process (required by ZMQ)
- **PUB socket** — bound to the module's assigned port from the port table
- **SUB sockets** — connected to upstream publishers via `subscribe()`
- **Interrupt PAIR** — enables thread-safe `stop()` from signal handlers
- **Poll loop** — `run()` blocks, dispatches messages to registered handlers

Usage pattern:
```cpp
MessageRouter router(Config{.moduleName="qwen_llm", .pubPort=5561});
router.subscribe(5571, "llm_prompt", false, [](auto& msg) { ... });
router.publishModuleReady();
router.run();  // blocks until stop()
```

For modules with callback threads (GStreamer ingest), `setOnPollCallback()` provides a hook to drain cross-thread queues on the poll thread.

Before MessageRouter, each module manually created its own ZMQ context, PUB socket, SUB sockets, and interrupt PAIR sockets. MessageRouter eliminates this boilerplate while preserving per-module process isolation.
...
| 5568 | VideoDenoise | `denoised_frame` | Yes |
| 5569 | AudioDenoise | `denoised_audio` | Yes |
| 5571 | Daemon | `module_status`, `llm_prompt`, `daemon_state` | No |

**Conflation rule:** `ZMQ_CONFLATE=1` on high-frequency data topics (video, audio, identity, denoised frames/audio) — only the latest message matters. `ZMQ_CONFLATE=0` on control topics (commands, transcription, LLM tokens) — every message must be delivered in order. When subscribing to multiple topics on one SUB socket, do NOT use conflation — use separate SUB sockets per conflated topic.

**ZMQ constants** (from `modules/zmq/include/zmq_transport/zmq_constants.hpp`):
- `kPublisherDataHighWaterMark = 2` — data-plane PUB sockets (video, audio)
- `kPublisherControlHighWaterMark = 100` — control-plane PUB sockets
- `kSubscriberControlHighWaterMark = 1000` — control SUB sockets
- `kPollTimeoutMs = 50` — `zmq::poll()` blocking timeout
...
| `oe_vlm` | WS Bridge (5570) | `ui_command` | `describe_scene` or `describe_uploaded_image` trigger |
...
| `oe_ws_bridge` | FaceRecog (5566) | `face_registered` | Face registration confirmations for `/chat` |
...
| `oe_ws_bridge` | VideoDenoise (5568) | `denoised_frame` | Denoised JPEG for `/denoise_video` |
| `oe_ws_bridge` | AudioDenoise (5569) | `denoised_audio` | Denoised PCM for `/denoise_audio` |
| `oe_video_denoise` | VideoIngest (5555) | `video_frame` | Frame input for denoising |
...
### 6.6 ZMQ Heartbeats (ZMTP Protocol-Level)
...
**On-demand enhancement models** (not counted in default budget — require additional VRAM headroom or eviction):

| Model | Quantization | VRAM | Inference Engine |
|:---|:---|---:|:---|
| BasicVSR++ | FP16 | 1.50 GB | ONNX Runtime CUDA EP |
| DTLN | FP32 | 0.05 GB | ONNX Runtime CUDA EP |

### 7.2 GPU Tier System

The daemon auto-detects available VRAM at boot via `probeGpu()` and selects the appropriate tier:

| Tier | VRAM | Active Modules | VLM Strategy | Enhancement (BasicVSR++ / DTLN) |
|:---|:---|:---|:---|:---|
| **Minimal** | 4 GB | STT + LLM (3B fallback) + TTS | Disabled | Disabled |
| **Balanced** | 8 GB | STT + LLM + TTS + Blur + FaceRecog | On-demand spawn, exit after use | DTLN on-demand only (50 MiB) |
| **Standard** | 12 GB | All core modules resident | Resident (periodic + on-demand) | Both on-demand (BasicVSR++ requires eviction) |
| **Ultra** | 16+ GB | All core modules + FP8 KV cache expanded | Resident, extended context | Both on-demand (headroom available) |
...
| Toggle video denoise (BasicVSR++) | +1,500 MB | On-demand ONNX model load; may trigger eviction on standard tier |
| Toggle audio denoise (DTLN) | +50 MB | Two lightweight ONNX models; fits within typical headroom |
...
|:---|:---|:---|:---|
| 0 | Video Denoise (on-demand) | 1.50 GB | Denoised video unavailable |
| 0 | Audio Denoise (on-demand) | 0.05 GB | Denoised audio unavailable |
| 1 | Background Blur | 0.50 GB | Cosmetic only (raw video passthrough) |
| 1 | Face Recognition | 0.50 GB | No identity in LLM prompt |
| 2 | VLM (on-demand) | 2.45 GB | `describe_scene` unavailable |
| 3 | TTS | 0.10 GB | Text-only responses |
| 4 | STT → Whisper Small | 1.00 GB | Higher WER |
| 5 | LLM | Never evicted | Core reasoning — always resident |
...
<details>
<summary><strong>BasicVSR++ FP16 (Video Denoise — On-Demand)</strong></summary>

| Direction | Field | Type | Shape | Notes |
|:---|:---|:---|:---|:---|
| Input | Video frames | `float16` | `[1, 5, 3, H, W]` | Temporal window of 5 BGR frames |
| Output | Denoised frame | `float16` | `[1, 3, H, W]` | Center frame denoised using temporal context |

**Architecture:** BasicVSR++ with bidirectional propagation and flow-guided deformable alignment. Processes a sliding window of 5 frames to exploit temporal redundancy for noise reduction. FP16 inference, ~1.50 GB VRAM. Spawned on-demand by daemon.

</details>

<details>
<summary><strong>DTLN FP32 (Audio Denoise — On-Demand)</strong></summary>

| Direction | Field | Type | Shape | Notes |
|:---|:---|:---|:---|:---|
| Input | Audio chunk | `float32` | `[1, 512]` | 32 ms frame at 16 kHz |
| Input | LSTM hidden state | `float32` | `[2, 1, 128]` | Carried across chunks |
| Output | Denoised audio | `float32` | `[1, 512]` | Cleaned audio frame |
| Output | LSTM hidden state | `float32` | `[2, 1, 128]` | Updated state for next chunk |

**Architecture:** Dual-signal Transformation LSTM Network. Two-stage: Stage 1 (separation network) operates in STFT magnitude domain; Stage 2 (signal reconstruction) operates on the estimated complex spectrum. Each stage uses 2-layer LSTM. Two separate ONNX models (~50 MiB total VRAM). Stateful — hidden states persist across audio chunks for temporal continuity.
...
| BasicVSR++ (PSNR, REDS4) | — | 31.7 dB (FP16) | — | — | **Baseline** |
| DTLN (PESQ) | — | 3.04 (FP32) | — | — | **Baseline** |
...
| Video Denoise | BasicVSR++ | EDVR, RealBasicVSR | Temporal window size vs. VRAM vs. quality |
| Audio Denoise | DTLN | RNNoise, DeepFilterNet | Model size vs. noise reduction quality |
...
The state machine is implemented in `modules/statemachine/include/statemachine/state_machine.hpp`. It supports up to 32 registered transitions (`kMaxTransitions`), each with optional guard functions (boolean preconditions) and action functions (side effects).

**7 States** (`enum class State : uint8_t`):

| State | Description | Active Modules |
|:---|:---|:---|
| **kIdle** | Waiting for user input | Video pipeline, face recognition |
| **kListening** | Recording user speech via PTT | + AudioIngest, VAD |
| **kProcessing** | STT → prompt assembly → LLM inference | + STT, LLM |
| **kProcessingVision** | VLM analyzing a video frame | + VLM |
| **kSpeaking** | LLM streaming tokens, TTS synthesizing | + LLM, TTS |
| **kInterrupted** | Barge-in detected, canceling generation | Flushing LLM + TTS |
| **kErrorRecovery** | Module crashed, restarting | Watchdog active |

**16 Events** (`enum class Event : uint8_t`): `kPttPress`, `kPttRelease`, `kPttCancel`, `kDescribeScene`, `kTextInput`, `kVadSilence`, `kLlmFirstSentence`, `kLlmComplete`, `kLlmTimeout`, `kVlmResult`, `kVlmTimeout`, `kTtsComplete`, `kModuleCrash`, `kModuleRecovered`, `kRecoveryTimeout`, `kInterruptDone`

**17 UI Actions** (`enum class UiAction : uint8_t`): Parsed from WebSocket commands via constexpr `parseUiAction()`. Includes `kStartAudio`, `kStopAudio`, `kEnableWebcam`, `kDisableWebcam`, `kPushToTalk`, `kTextInput`, `kDescribeScene`, `kTtsComplete`, `kSwitchMode`, `kCancelGeneration`, `kFlushTts`, `kStopPlayback`, `kRegisterFace`, `kSetImageAdjust`, `kSetBgMode`, `kUpdateShapes`, `kUnknown`.
...
The daemon is the **sole assembler** of LLM prompts via `PromptAssembler` (defined in `modules/statemachine/include/statemachine/prompt_assembler.hpp`). It fuses real-time context into a 4,096-token budget:
...
└── js/
    ├── ws.js           # 5 WebSocket channels, reconnect w/ exponential backoff
    ...
    ├── upload.js       # JPEG/PNG image upload → VLM description
    ├── denoise_video.js # Denoised video rendering (split-screen comparison)
    ├── denoise_audio.js # Denoised audio playback (A/B comparison with level bars)
...
|:---|:---|:---|
| `/video` (binary) | JPEG frames from BgBlur SHM | `Blob → createImageBitmap() → canvas.drawImage() + close()` at 30 fps |
| `/audio` (binary) | PCM float32 from TTS SHM | `Float32Array → AudioBuffer → AudioBufferSourceNode` at 24 kHz |
| `/chat` (JSON+binary) | Commands + events + image upload | JSON: route by `msg.type`. Binary: framed image upload `[4B json_len LE][JSON][image bytes]` for VLM description |
| `/denoise_video` (binary) | Denoised JPEG from VideoDenoise SHM | Rendered on second `<canvas>` for split-screen comparison with original |
| `/denoise_audio` (binary) | Denoised PCM from AudioDenoise SHM | Played via separate `AudioContext` for A/B comparison with raw audio |
...
| Video Denoise | Hide split-screen, disable denoise toggle | Enhancement unavailable, original video unaffected |
| Audio Denoise | Disable audio denoise toggle | Enhancement unavailable, original audio unaffected |
...
`oe_daemon` is the **only binary the user starts**. It is implemented as `OmniEdgeDaemon` in `modules/orchestrator/`. Its initialization sequence:

1. Parse `omniedge_config.yaml` via yaml-cpp
2. Probe GPU via `GpuProfiler::probe()` (NVML) → `selectTier()` → determine active modules per tier
3. Register all modules with `VramTracker` (budgets and eviction priorities from `vram_thresholds.hpp`)
4. Initialize `VramGate` with eviction callback (SIGTERM lowest-priority idle module)
5. Create `PromptAssembler` (4,096-token budget), `ModeOrchestrator` (VRAM-aware mode switching), `SessionPersistence` (atomic JSON save/load)
6. Create `MessageRouter` and register ZMQ subscriptions for all upstream topics
7. Spawn modules in dependency order via `ModuleLauncher` (`posix_spawn()`, WS Bridge first)
8. Wait for `module_ready` from each module (30 s timeout per module, graceful degradation on timeout)
9. Register state machine transitions (up to 32)
10. Enter blocking `zmq_poll()` loop — runs conversation FSM, watchdog, and periodic tasks

Key daemon components:
- **`ModuleLauncher`**: `posix_spawn()` wrapper with `spawnModule()`, `stopModule()` (SIGTERM → 5 s grace → SIGKILL), `restartModule()`, and `ModuleDescriptor` tracking (pid, restartCount, maxRestarts, evictPriority, vramBudgetMb, degraded, ready)
- **`ModeOrchestrator`**: Handles `switch_mode` commands with VRAM budget awareness. Modes: `full`, `dev-llm` (conversation only), `dev-video` (video only), `dev-stt-only` (audio→text)
- **`SessionPersistence`**: Atomic save/load of conversation state to `session_state.json`. Periodic save every 30 s. Rejects stale sessions older than 60 s on load.
...
10. oe_video_denoise   ← On-demand (user toggle)
11. oe_audio_denoise   ← On-demand (user toggle)
```

### 11.3 Module Readiness Protocol

Each module publishes `module_ready` (via `MessageRouter::publishModuleReady()`) after completing its startup sequence (load engine, allocate CUDA memory, initialize MessageRouter). The daemon collects these messages and transitions to operational state once all expected modules have reported. Modules that fail to report within 30 seconds are logged as warnings and the system proceeds without them (graceful degradation).
...
├── modules/
│   ├── common/                     # INTERFACE library: expected_result, signal_shutdown, time_utils, platform_detect, runtime_defaults
│   │   ├── cmake/                  # CMake helper modules
│   │   └── include/                # Cross-module headers (platform_detect.hpp, runtime_defaults.hpp, etc.)
│   ├── logger/                     # Static library: libomniedge_logger.a (OeLogger singleton)
│   ├── gpu/                        # Static library: libomniedge_gpu.a (GpuProfiler, CudaStream, PinnedStagingBuffer, GpuTier, VramInfo)
│   │   └── include/gpu/            # cuda_priority.hpp, gpu_tier.hpp, memory_constants.hpp, video_constants.hpp
│   ├── vram/                       # Static library: libomniedge_vram.a (VramTracker, VramGate)
│   │   └── include/vram/           # vram_thresholds.hpp (budgets, tier thresholds, eviction priorities)
│   ├── statemachine/               # Static library: libomniedge_statemachine.a (StateMachine, PromptAssembler, SessionPersistence)
│   │   └── include/statemachine/   # state_machine.hpp (7 states, 16 events), prompt_assembler.hpp, session_persistence.hpp
│   ├── zmq/                        # Static library: libomniedge_zmq.a (MessageRouter, ZmqPublisher, ZmqSubscriber, ShmMapping)
│   │   └── include/zmq_transport/  # port_settings.hpp, zmq_constants.hpp, shm_headers (Video, Audio, Jpeg, Vlm)
│   ├── gstreamer_ingest/           # Targets: omniedge_video_ingest, omniedge_audio_ingest (GStreamerPipeline, SileroVad)
│   ├── llm/                        # Target: omniedge_llm (QwenLLMNode, TensorRTLLMBackend)
│   ├── stt/                        # Target: omniedge_stt (WhisperSTTNode, TrtWhisperBackend, MelSpectrogram, HallucinationFilter)
│   ├── tts/                        # Target: omniedge_tts (KokoroTTSNode, OnnxKokoroBackend, SentenceSplitter)
│   ├── vlm/                        # Target: omniedge_vlm (MoondreamVLMNode, HybridMoondreamBackend, vision_subprocess.py)
│   ├── cv/                         # Targets: omniedge_bg_blur, omniedge_face_recog (YoloSegEngine, InspireFaceWrapper, GPU compositors)
│   ├── video_denoise/              # Target: omniedge_video_denoise (VideoDenoiseNode, OnnxBasicVsrppBackend) — on-demand
│   ├── audio_denoise/              # Target: omniedge_audio_denoise (AudioDenoiseNode, OnnxDtlnBackend) — on-demand
│   ├── orchestrator/               # Target: omniedge_daemon (OmniEdgeDaemon, ModeOrchestrator, ModuleLauncher)
│   └── ws_bridge/                  # Target: omniedge_ws_bridge (WebSocketBridgeNode, WsServer, IWsRelay)
...
├── log/                            # Runtime log directory (omniedge.log)
...
| ONNX Runtime (CUDA EP) | TTS (Kokoro), VLM decoder, VAD (Silero), Video Denoise (BasicVSR++), Audio Denoise (DTLN) |
| uWebSockets | WebSocket server (ws_bridge), 5-channel binary/JSON relay |
...
| tl::expected | C++23 `std::expected` polyfill (via `modules/common/include/expected_result.hpp`) |
...
All modules use the `OeLogger` singleton (defined in `modules/logger/include/logger/oe_logger.hpp`). Logs are written to a single consolidated file (`logs/omniedge.log`, configurable via `OE_LOG_DIR` environment variable).

**Logging API:**
```cpp
OeLogger::instance().setModule("whisper_stt");
OeLogger::instance().initFile();  // Opens logs/omniedge.log

OE_LOG_INFO("Transcription complete: {} tokens in {} ms", 42, 287);
OE_LOG_WARN("Hallucination filter rejected output");
OE_LOG_ERROR("TRT engine load failed: {}", errorMsg);
```

**Log format:**
```
2025-06-29 14:23:01.456 [INFO] [whisper_stt] [transcribe] Transcription complete: 42 tokens in 287 ms
```

Format: `YYYY-MM-DD HH:MM:SS.mmm [LEVEL] [module] [function] message`

**Constants:**
- `kConsolidatedLogFile = "omniedge.log"` — single shared log file for all modules
- `kDefaultLogDir = "logs"` — default directory, overridable via `OE_LOG_DIR`
- Each module session starts with a `kLogSessionSeparator` banner for visual separation

**Macros:** `OE_LOG_DEBUG`, `OE_LOG_INFO`, `OE_LOG_WARN`, `OE_LOG_ERROR` — use `std::format` for type-safe formatting. Thread-safe via internal mutex.
...
| BasicVSR++ FP16 | PSNR, SSIM (on REDS4 test set) | PSNR ≥ 30.0 dB, SSIM ≥ 0.85 |
| DTLN FP32 | PESQ, SI-SNR improvement | PESQ ≥ 2.8, SI-SNRi ≥ 8.0 dB |
...
<details>
<summary><strong>7. BasicVSR++ → ONNX FP16 (Video Denoise)</strong></summary>

```bash
# Download pre-trained BasicVSR++ checkpoint
huggingface-cli download ckkelvinn/basicvsrpp-reds --local-dir /models/basicvsrpp

# Export to ONNX (Python)
python3 scripts/export_basicvsrpp.py \
  --checkpoint /models/basicvsrpp/basicvsrpp_reds.pth \
  --output /models/onnx/basicvsrpp.onnx \
  --opset 17 --fp16

# Verify with ONNX Runtime
python3 -c "import onnxruntime as ort; sess = ort.InferenceSession('/models/onnx/basicvsrpp.onnx', providers=['CUDAExecutionProvider']); print('OK')"
```

Model uses bidirectional propagation with flow-guided deformable alignment. Input shape: `[1, 5, 3, H, W]` (5-frame temporal window). Output: denoised center frame.

</details>

<details>
<summary><strong>8. DTLN → ONNX FP32 (Audio Denoise)</strong></summary>

```bash
# Download pre-trained DTLN models
huggingface-cli download breizhn/DTLN --include "*.onnx" --local-dir /models/dtln

# Two models:
#   dtln_stage1.onnx — separation network (STFT magnitude domain)
#   dtln_stage2.onnx — signal reconstruction (complex spectrum domain)
# Both are already in ONNX format, no conversion needed.

# Verify
python3 -c "import onnxruntime as ort; ort.InferenceSession('/models/dtln/dtln_stage1.onnx', providers=['CUDAExecutionProvider']); print('Stage1 OK')"
python3 -c "import onnxruntime as ort; ort.InferenceSession('/models/dtln/dtln_stage2.onnx', providers=['CUDAExecutionProvider']); print('Stage2 OK')"
```

DTLN uses FP32 (no quantization) — the model is already small (~2.3M params, ~50 MiB VRAM). Quantization would risk degrading audio quality for negligible memory savings.

</details>

---

> **Document version:** 2.3 · Generated from codebase analysis  
