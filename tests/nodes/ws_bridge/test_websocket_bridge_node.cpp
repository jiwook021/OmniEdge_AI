...
public:
	...
	void broadcastBlurVideo(std::vector<uint8_t> data) override
	{
		lastBlurVideo = std::move(data);
	}
	...
	std::vector<uint8_t> lastBlurVideo;
...
TEST(WebSocketBridgeNodeTest, AcceptsSwitchModeSecurity)
{
	const int base = nextPortBase();
	WebSocketBridgeNode::Config cfg;
	cfg.wsBridgePubPort = base;
	...
	if (relayPtr->lastJson.is_object()) {
		EXPECT_NE(relayPtr->lastJson.value("type", ""), "error")
			<< "switch_mode security should be accepted. "
			<< "Error: " << relayPtr->lastJson.value("message", "");
	}
...
TEST(WebSocketBridgeNodeTest, AcceptsSwitchModeBeauty)
{
	const int base = nextPortBase();
	WebSocketBridgeNode::Config cfg;
	cfg.wsBridgePubPort = base;
	...
	if (relayPtr->lastJson.is_object()) {
		EXPECT_NE(relayPtr->lastJson.value("type", ""), "error")
			<< "switch_mode beauty should be accepted. "
			<< "Error: " << relayPtr->lastJson.value("message", "");
	}
...
TEST(WebSocketBridgeNodeTest, RejectsUnsupportedSwitchMode)
{
	const int base = nextPortBase();
	WebSocketBridgeNode::Config cfg;
	cfg.wsBridgePubPort = base;
	...
	ASSERT_TRUE(relayPtr->lastJson.is_object());
	EXPECT_EQ(relayPtr->lastJson.value("type", ""), "error");
	EXPECT_NE(relayPtr->lastJson.value("message", "").find("switch_mode.mode not supported"), std::string::npos);
