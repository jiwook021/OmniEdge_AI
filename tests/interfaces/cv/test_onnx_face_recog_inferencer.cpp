...
/// The underlying exception is stashed in the out-parameter so tests can log
/// the real ONNX Runtime failure reason instead of silently skipping.
bool tryLoadModel(OnnxFaceRecogInferencer& inferencer,
...
#define LOAD_MODEL_OR_SKIP(inferencer, dir) \
    do { \
        std::string _ep_err; \
        if (!tryLoadModel((inferencer), (dir), &_ep_err)) { \
            GTEST_SKIP() << "CUDA/TensorRT EP unavailable in this container: " << _ep_err; \
            return; \
        } \
    } while (0)
