...
# Runs inside the Docker dev container.  Run `bash run_all.sh --help` for usage.
#
# Host setup:
...
#
# Inside container:
#   bash run_all.sh --install                # install deps, build, launch
# ═══════════════════════════════════════════════════════════════════════════════
...
YAML_CONFIG="${PROJECT_ROOT}/config/omniedge_config.yaml"
...
build_binaries() {
    ...
    if [ ! -f build/build.ninja ] && [ ! -f build/Makefile ]; then
        ...
        local cmake_args=(
            ...
            -DBUILD_TESTING=OFF
            -DOE_USE_TRTLLM_CONVERSATION=ON
        )
    ...
    if cmake --build build -j"$safe_jobs" > "$LOG_DIR/build_default_targets.log" 2>&1; then
    ...
    local targets=(
        ...
        omniedge_face_filter omniedge_sam2
        omniedge_stt omniedge_tts omniedge_conversation
    )
    for t in "${targets[@]}"; do
...
launch_system() {
    ...
    # Set MODULES based on boot mode from INI
    local boot_mode
    boot_mode=$(ini_get "$INI_FILE" "daemon" "default_mode" "simple_llm")
    if [[ "$boot_mode" == "full_conversation" ]]; then
        MODULES="conversation_model video_ingest audio_ingest screen_ingest background_blur"
    else
        MODULES="conversation_model"
    fi
    ...
    LD_LIBRARY_PATH="$OE_RUNTIME_LD_LIBRARY_PATH" \
        stdbuf -oL "$ws_bin" --config "$YAML_CONFIG" >> "$omni_log" 2>&1 &
    WS_PID=$!
    sleep 2
    ...
    LD_LIBRARY_PATH="$OE_RUNTIME_LD_LIBRARY_PATH" \
        stdbuf -oL "$orch_bin" --config "$YAML_CONFIG" >> "$omni_log" 2>&1 &
    DAEMON_PID=$!
