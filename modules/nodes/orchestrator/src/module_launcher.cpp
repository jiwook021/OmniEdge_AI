	...
	std::chrono::seconds           readinessTimeout)
{
	...
	readinessSubscriber.set(zmq::sockopt::subscribe, std::string(kZmqTopicModuleReady));
...
// ── Pipeline Topological Launch ────────────────────────────────────────────

std::unordered_set<std::string> ModuleLauncher::launchPipeline(
	...
	std::chrono::seconds timeout)
{
	...
	std::vector<std::string> launched;  // for rollback on failure

	OE_LOG_INFO("pipeline_launch_start: name={}, stages={}",
	            pipeline.name, pipeline.stages.size());

	for (const auto& stage : pipeline.stages) {
		auto modIt = modules.find(stage.moduleName);
		if (modIt == modules.end()) {
			OE_LOG_ERROR("pipeline_launch_abort: module '{}' not registered",
			             stage.moduleName);
			failed.insert(stage.moduleName);
			break;
		}

		auto& desc = modIt->second;

		// Append pipeline-generated CLI args
		auto pipelineArgs = graph.argsForModule(pipeline.name, stage.moduleName);
		if (pipelineArgs) {
			for (const auto& arg : pipelineArgs->args) {
				desc.args.push_back(arg);
			}
		}

		// Spawn the stage
		auto spawnResult = spawnModule(desc);
		if (!spawnResult) {
			OE_LOG_ERROR("pipeline_stage_spawn_failed: pipeline={}, module={}, error={}",
			             pipeline.name, stage.moduleName, spawnResult.error());
			failed.insert(stage.moduleName);
			break;
		}

		launched.push_back(stage.moduleName);

		// Wait for module_ready before proceeding to next stage
		std::vector<ModuleDescriptor*> waitList{&desc};
		auto notReady = waitForReady(waitList, zmqCtx, timeout);

		if (!notReady.empty()) {
			OE_LOG_ERROR("pipeline_stage_ready_timeout: pipeline={}, module={}",
			             pipeline.name, stage.moduleName);
			failed.insert(stage.moduleName);
			break;
		}

		OE_LOG_INFO("pipeline_stage_ready: pipeline={}, module={}, stage={}/{}",
		            pipeline.name, stage.moduleName,
		            launched.size(), pipeline.stages.size());
	}

	// Rollback on failure: stop all launched stages
	if (!failed.empty()) {
		OE_LOG_WARN("pipeline_launch_rollback: pipeline={}, stopping {} launched stages",
		            pipeline.name, launched.size());
		for (const auto& name : launched) {
			auto modIt = modules.find(name);
			if (modIt != modules.end()) {
				stopModule(modIt->second);
			}
		}
		// Return all stage names as failed
		for (const auto& stage : pipeline.stages) {
			failed.insert(stage.moduleName);
		}
	} else {
		OE_LOG_INFO("pipeline_launch_complete: pipeline={}, all {} stages ready",
		            pipeline.name, pipeline.stages.size());
	}
