...
void OmniEdgeDaemon::initialize()
{
	...
	// INI [profile_conversation] (the default boot profile).
	{
		...
		const std::array kGpuModules = {
			...
			// Security Camera mode — YOLO detector + on-demand VLM agent loop
			ModuleVramDef{"security_camera",     iniConfig_.vramBudgetMiB("security_camera",    kSecurityCameraMiB)},
			ModuleVramDef{"security_vlm",        iniConfig_.vramBudgetMiB("security_vlm",       kSecurityVlmMiB)},
			// Beauty mode — FaceMesh V2 + TPS warp + bilateral filter
			ModuleVramDef{"beauty",              iniConfig_.vramBudgetMiB("beauty",             kBeautyMiB)},
			// Video denoise (BasicVSR++) and face filter (AR overlay) — optional toggles
			ModuleVramDef{"video_denoise",       iniConfig_.vramBudgetMiB("video_denoise",      kBasicVsrppMiB)},
			ModuleVramDef{"face_filter",         iniConfig_.vramBudgetMiB("face_filter",        kFaceFilterMiB)},
		};
	...
	// Prompt assembler — token budgets from INI [prompt] section or compile-time defaults
	{
		...
		auto makeAssembler = [&](const char* sysPrompt) {
			return std::make_unique<PromptAssembler>(PromptAssembler::Config{
		...
		chatAssembler_  = makeAssembler(
			"You are OmniEdge AI, a helpful text-chat assistant. "
			"Respond concisely and naturally in written form.");
		voiceAssembler_ = makeAssembler(
			"You are OmniEdge AI, a helpful voice assistant. "
			"Respond concisely and naturally for spoken output.");

		const char* home = std::getenv("HOME");
		const std::string base = home ? std::string(home) + "/.omniedge"
		                              : std::string("/tmp/.omniedge");
		const std::string chatPath  = base + "/conversation_chat.json";
		const std::string voicePath = base + "/conversation_voice.json";
		chatAssembler_->loadFromFile(chatPath);
		chatAssembler_->setPersistPath(chatPath);
		voiceAssembler_->loadFromFile(voicePath);
		voiceAssembler_->setPersistPath(voicePath);
		OE_LOG_INFO("conversation_persistence_enabled: chat_path={} chat_turns={} voice_path={} voice_turns={}",
			chatPath, chatAssembler_->historySize(),
			voicePath, voiceAssembler_->historySize());
	}

	// Mode catalog — Five-Mode Architecture (Conversation, SAM2 Segmentation,
	// Vision Model, Security, Beauty). Per-module VRAM budgets live in
	// [vram_budgets] in omniedge.ini and are consumed by VramGate/executeSwitchPlan.
	modes_ = buildDefaultModeCatalog();
...
void OmniEdgeDaemon::loadModuleConfigFromYaml()
{
	...
	// Pure YAML → data parse (modules + conversation_models) via ModuleConfigLoader.
	{
		ModuleConfig parsed = parseYamlModuleConfig(
			root, order, kEvictPriorities,
			root["models_root"].as<std::string>(""));
		modules_                = std::move(parsed.modules);
		conversationModelFlags_ = std::move(parsed.conversationModelFlags);
	}
...
struct OmniEdgeDaemon::Transitions {
	...
	// T36: PROCESSING + TEXT_INPUT → PROCESSING (barge-in on active generation).
	// Cancel the in-flight LLM turn and re-submit the new prompt so users can
	// re-send rapidly without tripping "fsm_unhandled" warnings.
	std::optional<State> operator()(fsm::Processing, fsm::TextInput) const {
		OE_LOG_INFO("text_input_during_processing: cancelling current turn and re-submitting");
		if (d->messageRouter_) {
			d->messageRouter_->publish("conversation_prompt", nlohmann::json{
				{"v",    kSchemaVersion},
	...
	// T18b: SPEAKING + TEXT_INPUT → PROCESSING (text barge-in / new turn).
	// Cancels in-flight LLM generation + TTS playback, then starts the new turn.
	// Prevents chat UI from wedging when TTS is slow, crashes, or is disabled.
	std::optional<State> operator()(fsm::Speaking, fsm::TextInput) const {
		OE_LOG_INFO("text_input_during_speaking: cancelling tts and starting new turn");
		if (d->messageRouter_) {
			d->messageRouter_->publish("conversation_prompt", nlohmann::json{
				{"v",    kSchemaVersion},
				{"type", "cancel_generation"},
			});
			d->messageRouter_->publish("tts_cancel", nlohmann::json{
				{"v", kSchemaVersion},
			});
		}
		d->pendingSentence_.clear();
		d->buildAndPublishLlmPrompt(d->lastTranscription_);
		d->llmTimeoutDeadline_ = std::chrono::steady_clock::now() +
		                         std::chrono::seconds{d->config_.llmGenerationTimeoutS};
		return fsm::Processing{};
	...
	// T19b: SPEAKING → IDLE (LlmComplete without matching TtsComplete — TTS absent/unhealthy)
	std::optional<State> operator()(fsm::Speaking, fsm::LlmComplete) const {
		OE_LOG_WARN("llm_complete_in_speaking: no TtsComplete will follow, returning to idle");
		d->pendingSentence_.clear();
		d->fsmOnFinishTurn();
		return fsm::Idle{};
	}
...
struct CoverageProbe {
	...
	// T36
	std::optional<StateIndex> operator()(fsm::Processing, fsm::TextInput) const        { return StateIndex::kProcessing; }
	// T17/T18
	std::optional<StateIndex> operator()(fsm::Speaking, fsm::PttPress) const       { return StateIndex::kInterrupted; }
	// T18b
	std::optional<StateIndex> operator()(fsm::Speaking, fsm::TextInput) const      { return StateIndex::kProcessing; }
	// T19
	std::optional<StateIndex> operator()(fsm::Speaking, fsm::TtsComplete) const    { return StateIndex::kIdle; }
	// T19b
	std::optional<StateIndex> operator()(fsm::Speaking, fsm::LlmComplete) const    { return StateIndex::kIdle; }
	// T20
...
void OmniEdgeDaemon::run()
{
	...
	// If conversation_model was marked ready synchronously during launch
	// (its module_ready was seen by the launcher's scratch subscriber),
	// publish orchestrator_ready now so the frontend can enable the UI.
	maybePublishOrchestratorReady();
...
void OmniEdgeDaemon::handleMessage(const std::string& type, const nlohmann::json& msg)
{
	...
	if (type == "transcription")    { activeChannel_ = Channel::kVoice; lastTranscription_ = msg.value("text", ""); return; }
	if (type == "vad_status")       { if (!msg.value("speaking", true)) (void)applyFsmEvent(fsm::VadSilence{}); return; }
	if (type == "identity")         {
		const std::string n = msg.value("name", "");
		const float c = msg.value("confidence", 0.0f);
		if (chatAssembler_)  chatAssembler_->setFaceIdentity(n, c);
		if (voiceAssembler_) voiceAssembler_->setFaceIdentity(n, c);
		return;
	}
...
// == Orchestrator readiness gate (P2.1) =======================================
//
// Publishes orchestrator_ready exactly once, when conversation_model has
// reported module_ready. The frontend gates chat input and mode switches on
// receipt — it stays disabled until the backend is known to be serving.
void OmniEdgeDaemon::maybePublishOrchestratorReady()
{
	if (orchestratorReadyPublished_) return;
	if (!messageRouter_) return;

	auto it = moduleIndex_.find("conversation_model");
	if (it == moduleIndex_.end()) return;
	if (!modules_[it->second].ready) return;

	orchestratorReadyPublished_ = true;
	nlohmann::json msg = {
		{"v",    kSchemaVersion},
		{"type", "orchestrator_ready"},
	};
	messageRouter_->publish(std::string(kZmqTopicOrchestratorReady), msg);
	OE_LOG_INFO("orchestrator_ready_published");
}
...
void OmniEdgeDaemon::executeUiCommand(const nlohmann::json& cmd)
{
	...
	case UiAction::kPushToTalk: {
		...
		if (!sttRunning) {
			// Debounce: one autostart per 5 s so a keydown-repeat or UI bug
			// does not fork the binary dozens of times per second.
			const auto now = std::chrono::steady_clock::now();
			if (lastSttAutoStart_.time_since_epoch().count() != 0 &&
			    now - lastSttAutoStart_ < std::chrono::seconds{5}) {
				break;
			}
			lastSttAutoStart_ = now;
	...
	case UiAction::kTextInput: {
		...
		activeChannel_ = Channel::kChat;
		lastTranscription_ = cmd.value("text", "");
		(void)applyFsmEvent(fsm::TextInput{});
		break;
	}
	case UiAction::kDescribeUploadedImage: {
		if (!isLlmAvailable()) {
			publishError("Assistant is unavailable — LLM module is not running");
			return;
		}
		activeChannel_         = Channel::kChat;
		pendingImagePath_      = cmd.value("image_path", "");
		const std::string text = cmd.value("text", "Describe this image.");
		lastTranscription_     = text.empty() ? "Describe this image." : text;
		OE_LOG_INFO("describe_uploaded_image: image_path={}, text_len={}",
		           pendingImagePath_, lastTranscription_.size());
		(void)applyFsmEvent(fsm::TextInput{});
		break;
	}
	...
	case UiAction::kGetConversationHistory: {
		if (!messageRouter_ || !chatAssembler_ || !voiceAssembler_) break;
		nlohmann::json response = {
			{"v",     kSchemaVersion},
			{"type",  "conversation_history"},
			{"chat",  chatAssembler_->historyAsJson()},
			{"voice", voiceAssembler_->historyAsJson()},
		};
		messageRouter_->publish("conversation_history", response);
		OE_LOG_INFO("conversation_history_sent: chat_turns={} voice_turns={}",
		           chatAssembler_->historySize(), voiceAssembler_->historySize());
		break;
	}
	case UiAction::kClearConversationHistory: {
		const std::string channel = cmd.value("channel", std::string{"chat"});
		if (channel == "voice" && voiceAssembler_) {
			voiceAssembler_->clearHistory();
		} else if (chatAssembler_) {
			chatAssembler_->clearHistory();
		}
		OE_LOG_INFO("conversation_history_cleared: channel={}", channel);
		if (messageRouter_) {
			messageRouter_->publish("conversation_history", nlohmann::json{
				{"v",     kSchemaVersion},
				{"type",  "conversation_history"},
				{"chat",  chatAssembler_  ? chatAssembler_->historyAsJson()  : nlohmann::json::array()},
				{"voice", voiceAssembler_ ? voiceAssembler_->historyAsJson() : nlohmann::json::array()},
			});
		}
		break;
	}
	...
	case UiAction::kToggleSecurityMode: {
		...
		if (enabled) {
			if (switchToMode("security") && messageRouter_) {
...
void OmniEdgeDaemon::watchdogPoll()
{
		...
		for (auto& mod : modules_) {
			...
			if (silence > heartbeatTimeout) {
				...
				SubprocessManager::terminateProcess(
				    mod.pid, std::chrono::seconds{0}, /*killProcessGroup=*/true);
	...
	for (const auto& crash : crashes) {
		if (crash.pidWas > 0) {
			SubprocessManager::terminateProcess(
			    crash.pidWas, std::chrono::seconds{0}, /*killProcessGroup=*/true);
		}
	}
	...
	// Kept as raw waitpid: SubprocessManager::checkProcess is per-pid, but
	// here we need "reap any orphan" semantics that the primitive provides
	// directly via the -1 wildcard pid.
...
{
	if (!chatAssembler_ || !voiceAssembler_ || !messageRouter_) return;
	PromptAssembler& pa = promptAssembler(activeChannel_);
	...
	auto payload = pa.assemble(userText);
	if (!pendingImagePath_.empty()) {
		payload["image_path"] = pendingImagePath_;
		OE_LOG_INFO("conversation_prompt_with_image: image_path={}", pendingImagePath_);
...
void OmniEdgeDaemon::fsmOnFinishTurn()
{
	if (!lastTranscription_.empty()) {
		PromptAssembler& pa = promptAssembler(activeChannel_);
		pa.addToHistory(lastTranscription_, lastLlmResponse_);
	}
	lastTranscription_.clear();
...
nlohmann::json OmniEdgeDaemon::buildSessionState() const
{
	...
	return {
		...
		{"history_size_chat",   chatAssembler_  ? chatAssembler_->historySize()  : 0},
		{"history_size_voice",  voiceAssembler_ ? voiceAssembler_->historySize() : 0},
		{"modules",             modulesJson},
	};
}
	...
	const core::pipeline_orchestrator::SwitchPlan& plan)
{
	...
	for (const auto& vertexId : sortedEvictions) {
		...
		// Pinned modules (camera + mic + bridge) survive every mode switch —
		// tearing them down strands the frontend's /video socket and hands
		// the heartbeat watchdog a ~30 s silence window that ends in SIGKILL.
		if (desc.name == "video_ingest" || desc.name == "audio_ingest") {
			OE_LOG_INFO("graph_evict_skipped_pinned: module={}", desc.name);
			continue;
		}
		...
		if (savedPid > 0) {
			...
			for (int attempt = 0;
			     ...
			     ++attempt) {
				if (SubprocessManager::checkProcess(savedPid).exited) {
...
bool OmniEdgeDaemon::switchToMode(const std::string& requestedMode)
{
	...
	const auto publishFsmStateForMode = [this](std::string_view mode) {
	...
	// Apply interaction profile for the target mode. Unknown mode handling
	// remains in the graph-path checks below.
	applyProfileForMode(requestedMode);
	...
	publishFsmStateForMode(requestedMode);
...
void OmniEdgeDaemon::handleBootModeSwitch(const std::string& targetMode)
{
	...
	// Pinned modules stay running across every mode switch so the user-visible
	// audio/video feed never drops. The catalog already lists these in every
	// mode, but we force-add them to targetSet as a defence-in-depth against
	// a catalog entry that accidentally omits them.
	static const std::array<std::string_view, 2> kPinnedModules{
		"video_ingest", "audio_ingest",
	};
	for (const auto& pinned : kPinnedModules) {
		targetSet.insert(std::string(pinned));
	}
	...
	for (auto& mod : modules_) {
		if (currentSet.contains(mod.name) && !targetSet.contains(mod.name) && mod.isRunning()) {
			// Pinned modules survive every boot-mode switch (see executeSwitchPlan).
			if (mod.name == "video_ingest" || mod.name == "audio_ingest") {
				OE_LOG_INFO("boot_mode_skip_stop_pinned: module={}", mod.name);
				continue;
			}
...
void OmniEdgeDaemon::memoryWatchdog()
{
	...
	for (auto& mod : modules_) {
		...
		if (rssMb > limitMb) {
			...
			// SubprocessManager::terminateProcess handles SIGTERM→SIGKILL + bounded reap.
			SubprocessManager::terminateProcess(
			    mod.pid, std::chrono::seconds{0}, killGroup);
