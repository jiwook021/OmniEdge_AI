...
void OmniEdgeDaemon::initialize()
{
	...
	// INI [profile_conversation] (the default boot profile).
	{
		...
		const std::array kGpuModules = {
			...
			ModuleVramDef{"video_denoise",       iniConfig_.vramBudgetMiB("super_resolution",   kBasicVsrppMiB)},
		};
	...
	modeOrchestrator_ = std::make_unique<ModeOrchestrator>(ModeOrchestrator::Config{
		...
		.vramBudgetMiB = {
			...
			{"video_denoise",      iniConfig_.vramBudgetMiB("super_resolution",   kBasicVsrppMiB)},
		},
	});
	...
	messageRouter_->subscribe(kStt,       "transcription",   false, handler);
	...
	// If the default model needs companion STT or TTS, spawn them now.
	{
		...
		if (flagsIt != conversationModelFlags_.end()) {
			...
			if (flagsIt->second.needsStt) {
				auto sttIt = moduleIndex_.find("stt");
				if (sttIt != moduleIndex_.end()) {
					ModuleDescriptor& sttMod = modules_[sttIt->second];
					if (!sttMod.isRunning()) {
						auto sttSpawn = launcher_.spawnModule(sttMod);
						if (sttSpawn) {
							moduleHeartbeats_["stt"] = std::chrono::steady_clock::now();
							OE_LOG_INFO("companion_stt_auto_spawned: default model {} needs separate STT",
							          activeConversationModel_);
						}
					}
				}
...
void OmniEdgeDaemon::loadModuleConfigFromYaml()
{
	...
	// Store native_stt/native_tts flags per model so handleSelectConversationModel
	// can spawn companion STT/TTS modules data-driven (not hardcoded).
	...
	if (auto convModels = root["conversation_models"]; convModels && convModels.IsMap()) {
		...
		for (auto it = convModels.begin(); it != convModels.end(); ++it) {
			...
			const bool nativeStt = it->second["native_stt"].as<bool>(true);
			...
			OE_LOG_DEBUG("conversation_model_flags: model={}, needs_tts={}, needs_stt={}, dir={}",
			           modelName, !nativeTts, !nativeStt, resolvedDir);
		}
	}
...
void OmniEdgeDaemon::handleMessage(const std::string& type, const nlohmann::json& msg)
{
	...
	if (type == "heartbeat" && msg.contains("module")) {
		moduleHeartbeats_[msg["module"].get<std::string>()] =
...
void OmniEdgeDaemon::executeUiCommand(const nlohmann::json& cmd)
{
	...
	case UiAction::kToggleVlm:
		handleToggleModule(action, cmd);
		break;

	case UiAction::kVlmCapture: {
		// VLM capture: capture current camera frame and send to LLM for description
		// Reuses the describe_scene FSM path — the LLM handles vision natively
		if (!isLlmAvailable()) {
			publishError("Assistant is unavailable — LLM module is not running");
			break;
		}
	...
	case UiAction::kSwitchMode: {
		...
		// On every mode switch, the daemon sends SIGTERM to all heavy inference
		// processes from the previous mode, waits via waitpid to confirm
		// termination, verifies VRAM is freed through GpuProfiler, then spawns
		// the new mode's processes via posix_spawn and waits for module_ready.
		std::string requestedMode = cmd.value("mode", "");
		if (!requestedMode.empty() && modeOrchestrator_ && vramGate_) {
			// Apply the interaction profile for the target mode BEFORE switching
			// so eviction ordering reflects the new priorities.
			...
			if (!switchResult) {
				...
				// Transition FSM state for non-conversation modes
...
void OmniEdgeDaemon::handleToggleModule(UiAction action, const nlohmann::json& cmd)
{
	...
	static const std::unordered_map<UiAction, ToggleInfo> kToggleMap{
		...
		{UiAction::kToggleVlm,             {"vlm",              "video_ingest"}},
	};
...
void OmniEdgeDaemon::watchdogPoll()
{
	...
	constexpr std::chrono::milliseconds kVramWaitTimeout{kVramWaitTimeoutMs};
...
void OmniEdgeDaemon::buildAndPublishLlmPrompt(const std::string& userText)
{
	...
	// Gate on LLM readiness — do not publish if the LLM module has not yet
	...
	for (const auto& mod : modules_) {
		if (mod.name == "llm") {
			llmReady = mod.isRunning() && mod.ready;
			break;
		}
	}
	if (!llmReady) {
		OE_LOG_WARN("llm_prompt_deferred: LLM not ready, dropping prompt (len={})", userText.size());
		return;
	}

	auto payload = promptAssembler_->assemble(userText);
	OE_LOG_INFO("llm_prompt_published: text_len={}, payload_keys={}",
	           userText.size(), payload.size());
	messageRouter_->publish("llm_prompt", payload);
}
...
void OmniEdgeDaemon::publishSentenceToTts(const std::string& sentence)
{
	...
	for (const auto& mod : modules_) {
		if (mod.name == "tts") { ttsRunning = mod.isRunning(); break; }
	}
...
{
	if (messageRouter_) {
		messageRouter_->publish("llm_prompt", nlohmann::json{
			{"v",    kSchemaVersion},
...
void OmniEdgeDaemon::handleSelectConversationModel(const nlohmann::json& cmd)
{
	...
	// Publish model change to frontend
	if (messageRouter_) {
		messageRouter_->publish("module_status", nlohmann::json{
			{"v",      kSchemaVersion},
	...
	OE_LOG_INFO("conversation_model_active: model={}, needs_tts={}, vram_mib={}",
	          newModel, newNeedsTts, newBudgetMiB);
}
...
void OmniEdgeDaemon::handleUploadImage(const nlohmann::json& cmd)
{
	...
	if (modeOrchestrator_->currentMode() != "image_transform") {
		...
		auto switchCmd = nlohmann::json{
			{"v", 1}, {"type", "ui_command"},
			{"action", "switch_mode"}, {"mode", "image_transform"},
		};
		executeUiCommand(switchCmd);
		if (modeOrchestrator_->currentMode() != "image_transform") {
...
void OmniEdgeDaemon::handleStartImageTransform(const nlohmann::json& cmd)
{
	...
	// Set timeout for the diffusion inference
