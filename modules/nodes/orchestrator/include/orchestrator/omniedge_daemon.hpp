...
public:
	...
	enum class Channel { kChat, kVoice };
	[[nodiscard]] PromptAssembler& promptAssembler() noexcept { return promptAssembler(activeChannel_); }
	[[nodiscard]] PromptAssembler& promptAssembler(Channel ch) noexcept {
		return ch == Channel::kVoice ? *voiceAssembler_ : *chatAssembler_;
	}
	[[nodiscard]] PromptAssembler& chatAssembler() noexcept { return *chatAssembler_; }
	[[nodiscard]] PromptAssembler& voiceAssembler() noexcept { return *voiceAssembler_; }
...
private:
	...
	std::unique_ptr<PromptAssembler>      chatAssembler_;
	std::unique_ptr<PromptAssembler>      voiceAssembler_;
	Channel                               activeChannel_{Channel::kChat};
	...
	std::string pendingImagePath_;  ///< Set by kDescribeUploadedImage; consumed on next buildAndPublishLlmPrompt

	// ── Frontend readiness gate (P2.1) ──────────────────────────────────
	// Tracks which modules have sent module_ready. Once daemon finishes
	// startup and conversation_model is ready, publishes orchestrator_ready
	// exactly once. Frontend gates chat input on receipt.
	bool orchestratorReadyPublished_{false};
	void maybePublishOrchestratorReady();
	...
	std::string activeConversationModel_{"gemma_e2b"};

	// Per-model config flags parsed from conversation_models YAML section —
	// definition lives in orchestrator/module_config_loader.hpp so the
	// loader can return it by value.
	...
	std::chrono::steady_clock::time_point lastSttAutoStart_{};      ///< Debounce for push_to_talk auto-start
