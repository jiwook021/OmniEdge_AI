...
tl::expected<void, std::string> TTSNode::configureTransport()
{
	...
	*ttsHeader = ShmTtsHeader{};
	...
	messageRouter_.subscribe(config_.subLlmPort, kZmqTopicConversationResponse, false,
		[this](const nlohmann::json& msg) { handleLlmResponse(msg); });
	messageRouter_.subscribe(config_.subDaemonPort, kZmqTopicModuleStatus, false,
