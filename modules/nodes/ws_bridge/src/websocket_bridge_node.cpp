...
{
	return mode == "voice" || mode == "conversation"
...
void WebSocketBridgeNode::handleBinaryUpload(std::string_view binaryFrame)
{
	...
	if (jsonLen == 0 || kHeaderSize + jsonLen > binaryFrame.size()) {
...
void WebSocketBridgeNode::setMode_(std::string_view mode)
{
	...
	// activeTopics_ is read by messageRouter_.run() handlers on the relay thread: guard the write.
	{
		const std::lock_guard<std::mutex> lock(activeMutex_);
...
static std::vector<uint8_t> compressBgrToJpeg(
	const uint8_t* bgr, uint32_t w, uint32_t h) noexcept
{
	...
	const nlohmann::json& payload)
{
	...
	if (reader.writePos() == 0) return {};   // no frames yet
