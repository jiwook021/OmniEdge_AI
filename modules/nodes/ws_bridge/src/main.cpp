...
int main(int argc, char* argv[])
{
	...
	YAML::Node yaml = oe::main::loadYaml(configPath, "omniedge_ws_bridge");
	if (!yaml) return 1;
	...
	if (yaml["sam2"] && yaml["sam2"]["zmq_pub_port"]) {
		cfg.sam2SubPort = yaml["sam2"]["zmq_pub_port"].as<int>(cfg.sam2SubPort);
	}
