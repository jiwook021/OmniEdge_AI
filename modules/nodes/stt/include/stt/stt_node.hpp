 ...
 * otherwise onLoadInferencer() returns an error.
 */
...
public:
	...
	struct Config {
		...
		int zmqHeartbeatIvlMs       = kIntervalMs;  ///< Heartbeat interval
		int zmqHeartbeatTimeToLiveMs = kTimeoutMs;  ///< Heartbeat TTL
		int zmqHeartbeatTimeoutMs   = kTimeoutMs;   ///< Heartbeat timeout
		...
		// -- Module identity --------------------------------------------------
		std::string moduleName = "stt";  ///< Matches YAML key for logging
	 ...
	 *
	 * Must be called before initialize(). If not called, onLoadInferencer()
	 * returns an error.
	 */
...
private:
	...
	// -----------------------------------------------------------------------
	// Processing pipeline (created in onConfigure(), used in run())
	// -----------------------------------------------------------------------
