...
tl::expected<void, std::string> STTNode::onConfigure()
{
	OE_ZONE_SCOPED;
	...
	// 3 — High-priority CUDA stream
	{
		int leastPriority = 0, greatestPriority = 0;
		OE_CUDA_CHECK(cudaDeviceGetStreamPriorityRange(&leastPriority,
		                                               &greatestPriority));
		const int priority = std::max(config_.cudaStreamPriority,
		                              greatestPriority);
		OE_CUDA_CHECK(cudaStreamCreateWithPriority(&cudaStream_,
	...
	for (int attempt = 1; attempt <= 10; ++attempt) {
		try {
			audioInputShm_ = std::make_unique<ShmMapping>(
				config_.inputShmName, kAudioShmSegmentByteSize, /*create=*/false);
			break;
			...
			OE_LOG_WARN("stt_shm_retry: attempt={}/10, waiting for {}: {}",
...
tl::expected<void, std::string> STTNode::onLoadInferencer()
{
	OE_ZONE_SCOPED;
