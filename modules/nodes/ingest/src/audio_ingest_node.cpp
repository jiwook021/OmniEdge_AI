...
tl::expected<void, std::string> AudioIngestNode::configureTransport()
{
	OE_ZONE_SCOPED;
	...
	messageRouter_.setOnPollCallback([this]() -> bool {
		...
		// Poll GStreamer bus for EOS (file source finished) or pipeline errors.
		// If the pipeline reached EOS while VAD was in the speaking state,
		// force a silence transition so downstream consumers (ConversationNode)
		// can flush accumulated audio and run transcription.
		if (pipeline_ && pipelineActive_.load(std::memory_order_acquire)
		    && pipeline_->checkBusErrors())
		{
			{
				std::lock_guard<std::mutex> lk(stateMtx_);
				if (vadSpeaking_) {
					OE_LOG_INFO("audio_eos_flush: forcing silence transition after pipeline EOS");
					vadSpeaking_    = false;
					silenceSamples_ = 0;
					pendingVadMsgs_.push_back({false, config_.silenceDurationMs});
				}
			}
...
tl::expected<void, std::string> AudioIngestNode::loadInferencer()
{
	OE_ZONE_SCOPED;

	// GStreamer pipeline — four source modes:
	//   1. "auto"  → autoaudiosrc (local mic via PulseAudio/ALSA — Docker/native Linux)
	//   2. WAV file → filesrc + wavparse (integration tests)
	//   3. Raw PCM file → filesrc + rawaudioparse (integration tests)
	//   4. "tcp"   → tcpclientsrc (Windows host audio via WSL2)
	std::string pipelineStr;

	auto isAudioFile = [](const std::string& path) -> bool {
		const std::filesystem::path p(path);
		const auto ext = p.extension().string();
		return ext == ".wav" || ext == ".raw" || ext == ".pcm";
	};

	if (config_.audioSource == "auto") {
		// Direct local microphone capture (Docker / native Linux)
		...
		OE_LOG_INFO("audio_ingest_autoaudiosrc: rate={}", config_.sampleRateHz);
	} else if (isAudioFile(config_.audioSource) &&
	           std::filesystem::exists(config_.audioSource))
	{
		const std::filesystem::path audioPath(config_.audioSource);
		const auto ext = audioPath.extension().string();
		...
		if (ext == ".wav") {
			pipelineStr = std::format(
				...
				"! audio/x-raw,format=F32LE,rate={},channels=1 "
				"! appsink name=audio_sink drop=true max-buffers=2",
				config_.audioSource, config_.sampleRateHz);
		} else {
			// Raw F32LE PCM or .pcm
			pipelineStr = std::format(
				"filesrc location=\"{}\" "
				"! audio/x-raw,format=F32LE,rate={},channels=1 "
				"! audioconvert ! audioresample "
		...
		OE_LOG_INFO("audio_ingest_filesrc: path={}, rate={}",
			config_.audioSource, config_.sampleRateHz);
	} else {
		// TCP stream from Windows host (WSL2 production mode)
		pipelineStr = std::format(
...
void AudioIngestNode::drainPendingPublishes()
{
...

	{
		...
		// Re-reserve: swap() transferred the pre-allocated capacity to the
		// local vectors, leaving pendingChunks_/pendingVadMsgs_ at capacity 0.
		// Without this, every push_back() from onAudioBuffer() would allocate.
		pendingChunks_.reserve(64);
		pendingVadMsgs_.reserve(64);
	}
...
{
	static thread_local AudioChunkMsg msg;
	msg.slot    = slot;
	msg.samples = samples;
	msg.seq     = seq;
	msg.ts      = static_cast<int64_t>(ts);
	messageRouter_.publish("audio_chunk", nlohmann::json(msg));
}
