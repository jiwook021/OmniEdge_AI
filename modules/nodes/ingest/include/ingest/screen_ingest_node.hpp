...
class ScreenIngestSnapshotTest;  // seeds lastJpeg_ cache for snapshot round-trip test

class ScreenIngestNode : public ModuleNodeBase<ScreenIngestNode> {
public:
    friend class ModuleNodeBase<ScreenIngestNode>;
    friend class ScreenIngestSnapshotTest;
...
private:
    ...
    // ── Snapshot cache (same mutex as pending*, survives processPendingFrame) ─
    //
    // pendingJpeg_ is swapped-empty after each decode, so a `snapshot` command
    // that arrives between frames would see no data. lastJpeg_ holds the most
    // recent JPEG for on-demand responses to the `snapshot` UI action.
    std::vector<uint8_t> lastJpeg_;
    uint32_t             lastJpegWidth_{0};
    uint32_t             lastJpegHeight_{0};
    ...
    /// Handles `snapshot` UI action — publishes the cached JPEG as a
    /// base64-encoded JSON payload on `kZmqTopicSnapshotResponse`.
    /// Called on the poll thread (same thread as publish), so no extra sync.
