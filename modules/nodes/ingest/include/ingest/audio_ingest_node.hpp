...
//   1. audio_source: "auto"  → autoaudiosrc (local mic via PulseAudio/ALSA)
//   2. audio_source: <file>  → filesrc (WAV/PCM for integration tests)
//   3. audio_source: "tcp"   → tcpclientsrc (Windows host → WSL2):
...
public:
	...
	struct Config {
		// GStreamer source: "auto" (local mic), "tcp" (Windows host), or file path
		std::string audioSource         = "tcp";
		std::string windowsHostIp       = "172.17.0.1";
		int         audioTcpPort        = kAudioIngestTcpPort;
