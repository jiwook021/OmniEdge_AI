...
private:
	...
	void handleUiCommand(UiAction action, const nlohmann::json& msg);

	/** Apply brightness/contrast/saturation to BGR24 buffer in-place. */
	void applyImageAdjust(uint8_t* bgr, std::size_t pixelCount) const;
	...
	// Image-adjust parameters, updated from ui_command handler thread,
	// read in onVideoFrame on the GStreamer streaming thread.
	// Defaults match the UI neutral positions (no-op fast path).
	std::atomic<float>                  brightness_{0.0f};   // -100..100 additive
	std::atomic<float>                  contrast_{1.0f};     //  0.5..3.0 multiplicative around 128
	std::atomic<float>                  saturation_{1.0f};   //  0.0..2.0 gray-blend factor
