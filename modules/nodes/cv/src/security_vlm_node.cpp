...
void SecurityVlmNode::drainPendingIncidents()
{
    ...
    while (!pending_.empty() && pending_.front().dispatchAt <= now) {
        ...
        if (auto check = ensureVramAvailableMiB(kSecurityVlmInferenceHeadroomMiB);
            !check) {
            OE_LOG_WARN("security_vlm_vram_preflight_failed: event_id={} reason={}",
                        inc.eventId, check.error());
            appendIncidentJsonl(nlohmann::json{
                {"event_id", inc.eventId},
                {"source_event", inc.source},
                {"status", "vram_preflight_failed"},
                {"reason", check.error()},
            });
            continue;
        }
