...
tl::expected<void, std::string> BackgroundBlurNode::onConfigure()
{
	OE_ZONE_SCOPED;
	...
	for (int attempt = 1; attempt <= 10; ++attempt) {
		try {
			...
			OE_LOG_WARN("bg_blur_shm_retry: attempt={}/10, waiting for {}: {}",
...
tl::expected<void, std::string> BackgroundBlurNode::onLoadInferencer()
{
	OE_ZONE_SCOPED;
...
void BackgroundBlurNode::publishBlurredFrame(std::size_t jpegSize, bool blurred)
{
	...
	messageRouter_.publish("blurred_frame", payload);
}
