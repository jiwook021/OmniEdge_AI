...
tl::expected<void, std::string> FaceRecognitionNode::configureTransport()
{
	...
	// Input SHM (consumer — retry up to 10 times waiting for producer)
	auto shmResult = oe::cv::openInputShm(config_.inputShmName, "face_recog");
	...
	OE_LOG_INFO("face_recog_configured: pub={}, model={}, faces={}, threshold={}, subsample={}, input_topic={}",
		config_.pubPort, config_.modelPackPath,
		static_cast<int>(knownFaces_.size()),
		config_.recognitionThreshold, config_.frameSubsample, config_.inputTopic);
