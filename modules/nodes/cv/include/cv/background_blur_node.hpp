...
//   → [MediaPipe Selfie Seg person mask on GPU]
...
//   → [MediaPipe Selfie Seg + blur + composite on GPU]
//   → /oe.cv.blur.bgr (circular-buffer SHM, BGR24)
//   → ZMQ PUB "blur_bgr_frame" on port 5567
//
...
public:
	...
	struct Config {
		...
		// SHM — output
		std::string outputShmName = "/oe.cv.blur.jpeg";   ///< JPEG mode default
		std::string outputBgrShmName = "/oe.cv.blur.bgr"; ///< BGR24 mode default
		...
		std::string bgrTopic = "blur_bgr_frame";
...
private:
	...
	// SHM
	std::unique_ptr<ShmMapping> shmIn_;   ///< input (consumer, any BGR24 circular buffer)
	...
	// Publish "blurred_frame" ZMQ notification (JPEG mode)
	void publishBlurredFrame(std::size_t jpegSize, bool blurred = true);

	// Publish "blur_bgr_frame" ZMQ notification (BGR24 mode)
};
