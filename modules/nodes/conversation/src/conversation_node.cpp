...
void ConversationNode::handlePrompt(const nlohmann::json& msg)
{
    ...
    if (videoConversationEnabled_ && videoShm_ &&
        inferencer_->supportsNativeVision()) {
        auto frameView = readLatestBgrFrame(*videoShm_);
        if (frameView.data != nullptr && frameView.width > 0 && frameView.height > 0) {
            ...
            result = inferencer_->generateWithVideo(
                prompt,
                std::span<const uint8_t>(frameView.data, frameBytes),
                frameView.width,
                frameView.height,
                params,
                tokenCallback);
...
void ConversationNode::handleVideoConversationToggle(const nlohmann::json& msg)
{
    ...
    if (enabled && !videoShm_) {
        OE_LOG_WARN("OE-CONV-3014: video conversation requested but video SHM not available");
        return;
    }

    videoConversationEnabled_ = enabled;
    OE_LOG_INFO("video_conversation_toggled: enabled={}", enabled);
}
