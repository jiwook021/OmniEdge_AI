...
tl::expected<void, std::string> AudioDenoiseNode::configureTransport()
{
	OE_ZONE_SCOPED;
...
tl::expected<void, std::string> AudioDenoiseNode::loadInferencer()
{
	OE_ZONE_SCOPED;
...
void AudioDenoiseNode::handleAudioChunk(const nlohmann::json& chunkMetadata)
{
	...
	// Use atomic_ref with acquire semantics to prevent compiler reordering
	// of the seqNumber reads relative to the data copy.
	const auto* audioSlotHeader = reinterpret_cast<const ShmAudioHeader*>(slotBase);
	const uint64_t seqBefore =
		std::atomic_ref<const uint64_t>(audioSlotHeader->seqNumber)
	...
	static thread_local DenoisedAudioMsg payload;
	payload.shm     = config_.shmOutput;
	payload.samples = static_cast<int64_t>(denoisedPcm.size());
	payload.seq     = seqNumber_;
	messageRouter_.publish("denoised_audio", nlohmann::json(payload));
}
