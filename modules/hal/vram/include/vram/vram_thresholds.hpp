...
//
// GPU Mode Architecture — Five Mutually Exclusive Modes:
...
//   Mode 4 — Vision Model:           LLaMA 3.2 11B Vision (INT4-AWQ)
//
...
inline constexpr std::size_t kLlamaVision11bMiB = 6500; ///< LLaMA 3.2 11B Vision INT4-AWQ weights
...
inline constexpr std::size_t kLlamaVision11bInferenceHeadroomMiB = 1024; ///< VLM image encoder + cross-attention + KV cache
...
enum class InteractionProfile : uint8_t {
    ...
    kVisionModel,        ///< LLaMA 3.2 11B Vision (INT4-AWQ) — text+vision input, text output
};
