...
public:
     ...
     */
    /// Compute minimum segment size for the given slot size.
    [[nodiscard]] static constexpr std::size_t requiredSize(std::size_t slotSize) noexcept
    {
        return kDoubleBufferControlBytes + sizeof(Header) + 2 * slotSize;
    }

    explicit ShmDoubleBuffer(std::string_view name, std::size_t size, bool create)
        : mapping_(name, size, create)
    {
        constexpr std::size_t kMinSize = kDoubleBufferControlBytes + sizeof(Header);
        if (size < kMinSize) {
            throw std::invalid_argument(
                std::format("[ShmDoubleBuffer] segment '{}' size {} < minimum {} "
                            "(control={} + header={})",
                            name, size, kMinSize,
                            kDoubleBufferControlBytes, sizeof(Header)));
        }
    ...
    struct ReadResult {
        const uint8_t* data;
        uint32_t       slot;
        uint64_t       sequence;  ///< raw writeIndex counter (for post-copy guard)
        bool           valid;     ///< false if producer flipped during pointer computation
    };

    /**
     * @brief Get the data region for the read slot (consumer side).
     *
     * IMPORTANT: The `valid` flag only indicates the slot was stable during
     * pointer computation.  After copying data, the consumer MUST re-check
     * by calling writeIndex() again — if it changed, the copy may be torn.
     */
    [[nodiscard]] ReadResult
    readSlotData(std::size_t slotSize, std::size_t dataOffset) const noexcept
    {
        const uint64_t rawIdx = controlIndex_->load(std::memory_order_acquire);
        const uint32_t slot = static_cast<uint32_t>(rawIdx % 2);
        const auto* slotPtr = mapping_.bytes() + dataOffset + slot * slotSize;

        const uint64_t rawIdxAfter = controlIndex_->load(std::memory_order_acquire);

        return {slotPtr, slot, rawIdx, rawIdx == rawIdxAfter};
    }
