...
//   [ShmVideoHeader (64 B)][ShmCircularControl (128 B)][slot 0 BGR24]...[slot N-1]
...
inline BgrFrameView readLatestBgrFrame(const ShmMapping& shm)
{
    ...
    if (!slot) return {};  // nothing written yet
...
inline std::optional<uint32_t> currentShmSlotIndex(const ShmMapping& shm)
{
    ShmCircularReader reader(shm, sizeof(ShmVideoHeader));
