...
void MessageRouter::publishModuleReady()
{
    ...
    publisher_->publish(kZmqTopicModuleReady, {
        {"v",      kSchemaVersion},
...
// ---------------------------------------------------------------------------
// publishHeartbeat() — callable from blocking handlers
// ---------------------------------------------------------------------------

void MessageRouter::publishHeartbeat()
{
    auto now = std::chrono::steady_clock::now();
    if (now - lastHeartbeat_ < std::chrono::milliseconds{kIntervalMs}) {
        return;  // Not time yet — avoid flooding.
    }
    ...
    publisher_->publish(kZmqTopicHeartbeat, {
        {"type",             std::string(kZmqTopicHeartbeat)},
        {"module",           config_.moduleName},
        {"pid",              static_cast<int>(::getpid())},
        {"uptime_s",         uptimeS},
...
void MessageRouter::stop() noexcept
{
    ...
    try {
        ...
        OE_LOG_WARN("OE-ZMQ-2001 message_router_stop_interrupt_swallowed: {}", e.what());
    }
}
