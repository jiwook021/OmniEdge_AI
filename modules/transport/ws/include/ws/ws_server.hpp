...
//
// RAII wrapper around a uWebSockets uWS::App.  Manages ten WebSocket
// endpoints and two HTTP endpoints on a single port:
//
//   WS  /video          — binary ArrayBuffer (raw JPEG frames from VideoIngest).
//                         Server → browser only.  Always mounted — never replaced
//                         by downstream CV overlays.
...
//   WS  /sam2_video     — binary ArrayBuffer (SAM2 mask JPEG overlay from Sam2Node).
//                         Server → browser only.  Active when SAM2 segmentation triggered.
//   WS  /screen_video   — binary ArrayBuffer (screen capture JPEG from ScreenIngestNode).
//                         Server → browser only.  Active when screen source is selected.
//   WS  /security_video — binary ArrayBuffer (annotated JPEG from SecurityCameraNode).
//                         Server → browser only.  Active when security mode is active.
//   WS  /beauty_video   — binary ArrayBuffer (beauty-processed JPEG from BeautyNode).
//                         Server → browser only.  Active when beauty mode is active.
//   WS  /blur_video     — binary ArrayBuffer (blurred JPEG from BackgroundBlurNode).
//                         Server → browser only.  Active when background blur is on.
//
...
public:
	...
	struct Config {
		...
		std::string recordingsDir;                   ///< Security recordings dir (served at /recordings/*)
	};
	...
	/**
	 * @brief Broadcast a SAM2 mask JPEG overlay to all /sam2_video clients.
	 *
	 * Thread-safe. Used by Sam2Node interactive segmentation.
	 * @param data  JPEG bytes (mask composited onto original frame).
	 */
	void broadcastSam2Video(std::vector<uint8_t> data);

	/**
	 * @brief Broadcast a screen capture JPEG to all /screen_video clients.
	 *
	 * Thread-safe. Used by ScreenIngestNode screen capture relay.
	 * @param data  JPEG bytes.
	 */
	void broadcastScreenVideo(std::vector<uint8_t> data);

	/**
	 * @brief Broadcast annotated security JPEG to all /security_video clients.
	 *
	 * Thread-safe. Used by SecurityCameraNode detection overlay relay.
	 * @param data  JPEG bytes (annotated with bounding boxes).
	 */
	void broadcastSecurityVideo(std::vector<uint8_t> data);

	/**
	 * @brief Broadcast beauty-processed JPEG to all /beauty_video clients.
	 *
	 * Thread-safe. Used by BeautyNode face beautification relay.
	 * @param data  JPEG bytes.
	 */
	void broadcastBeautyVideo(std::vector<uint8_t> data);

	/**
	 * @brief Broadcast a background-blurred JPEG frame to all /blur_video clients.
	 *
	 * Thread-safe. Used by BackgroundBlurNode relay so the blurred preview is
	 * delivered on its own channel and does not compete with the raw /video tile.
	 * @param data  JPEG bytes.
	 */
	void broadcastBlurVideo(std::vector<uint8_t> data);
