...
struct WsServer::Impl {
	...
	std::vector<WsHandle*> sam2VideoClients;
	std::vector<WsHandle*> screenVideoClients;
	std::vector<WsHandle*> securityVideoClients;
	std::vector<WsHandle*> beautyVideoClients;
	std::vector<WsHandle*> blurVideoClients;
	...
	: impl_(std::make_unique<Impl>())
{
	...
	// ── /sam2_video endpoint: SAM2 mask JPEG overlay, server → client ──────
	impl_->app->ws<PerSocketData>("/sam2_video", {
		.compression = uWS::DISABLED,
		.maxPayloadLength = 0,
		.idleTimeout = 0,
		.open = [this](Impl::WsHandle* ws) {
			impl_->sam2VideoClients.push_back(ws);
			OE_LOG_INFO("ws_client_connected: channel=sam2_video, clients={}",
				static_cast<int>(impl_->sam2VideoClients.size()));
		},
		.close = [this](Impl::WsHandle* ws, int code, std::string_view /*msg*/) {
			auto& v = impl_->sam2VideoClients;
			v.erase(std::remove(v.begin(), v.end(), ws), v.end());
			OE_LOG_INFO("ws_client_disconnected: channel=sam2_video, code={}", code);
		}
	});

	// ── /screen_video endpoint: screen capture JPEG, server → client ──────
	impl_->app->ws<PerSocketData>("/screen_video", {
		.compression = uWS::DISABLED,
		.maxPayloadLength = 0,
		.idleTimeout = 0,
		.open = [this](Impl::WsHandle* ws) {
			impl_->screenVideoClients.push_back(ws);
			OE_LOG_INFO("ws_client_connected: channel=screen_video, clients={}",
				static_cast<int>(impl_->screenVideoClients.size()));
		},
		.close = [this](Impl::WsHandle* ws, int code, std::string_view /*msg*/) {
			auto& v = impl_->screenVideoClients;
			v.erase(std::remove(v.begin(), v.end(), ws), v.end());
			OE_LOG_INFO("ws_client_disconnected: channel=screen_video, code={}", code);
		}
	});

	// ── /security_video endpoint: annotated security JPEG, server → client ──
	impl_->app->ws<PerSocketData>("/security_video", {
		.compression = uWS::DISABLED,
		.maxPayloadLength = 0,
		.idleTimeout = 0,
		.open = [this](Impl::WsHandle* ws) {
			impl_->securityVideoClients.push_back(ws);
			OE_LOG_INFO("ws_client_connected: channel=security_video, clients={}",
				static_cast<int>(impl_->securityVideoClients.size()));
		},
		.close = [this](Impl::WsHandle* ws, int code, std::string_view /*msg*/) {
			auto& v = impl_->securityVideoClients;
			v.erase(std::remove(v.begin(), v.end(), ws), v.end());
			OE_LOG_INFO("ws_client_disconnected: channel=security_video, code={}", code);
		}
	});

	// ── /beauty_video endpoint: beauty-processed JPEG, server → client ──────
	impl_->app->ws<PerSocketData>("/beauty_video", {
		.compression = uWS::DISABLED,
		.maxPayloadLength = 0,
		.idleTimeout = 0,
		.open = [this](Impl::WsHandle* ws) {
			impl_->beautyVideoClients.push_back(ws);
			OE_LOG_INFO("ws_client_connected: channel=beauty_video, clients={}",
				static_cast<int>(impl_->beautyVideoClients.size()));
		},
		.close = [this](Impl::WsHandle* ws, int code, std::string_view /*msg*/) {
			auto& v = impl_->beautyVideoClients;
			v.erase(std::remove(v.begin(), v.end(), ws), v.end());
			OE_LOG_INFO("ws_client_disconnected: channel=beauty_video, code={}", code);
		}
	});

	// ── /blur_video endpoint: background-blurred JPEG, server → client ──────
	impl_->app->ws<PerSocketData>("/blur_video", {
		.compression = uWS::DISABLED,
		.maxPayloadLength = 0,
		.idleTimeout = 0,
		.open = [this](Impl::WsHandle* ws) {
			impl_->blurVideoClients.push_back(ws);
			OE_LOG_INFO("ws_client_connected: channel=blur_video, clients={}",
				static_cast<int>(impl_->blurVideoClients.size()));
		},
		.close = [this](Impl::WsHandle* ws, int code, std::string_view /*msg*/) {
			auto& v = impl_->blurVideoClients;
			v.erase(std::remove(v.begin(), v.end(), ws), v.end());
			OE_LOG_INFO("ws_client_disconnected: channel=blur_video, code={}", code);
		}
	});
	...
	impl_->app->get("/status", [this](uWS::HttpResponse<false>* res,
		...
		nlohmann::json doc = {
			...
			{"sam2_video_clients",     static_cast<int>(impl_->sam2VideoClients.size())},
			{"screen_video_clients",   static_cast<int>(impl_->screenVideoClients.size())},
			{"security_video_clients", static_cast<int>(impl_->securityVideoClients.size())},
			{"beauty_video_clients",   static_cast<int>(impl_->beautyVideoClients.size())},
			{"blur_video_clients",     static_cast<int>(impl_->blurVideoClients.size())},
		};
	...
	// ── HTTP GET /recordings/* — serve security MP4 recordings ────────────
	impl_->app->get("/recordings/*", [this](uWS::HttpResponse<false>* res,
	                                         uWS::HttpRequest* req) {
		if (impl_->config.recordingsDir.empty()) {
		...
		// Strip "/recordings/" prefix to get the filename.
		std::string filename(urlPath.substr(12));  // len("/recordings/") = 12
		...
		std::string filePath = impl_->config.recordingsDir + "/" + filename;
		auto fileResult = readBinary(filePath);
		if (!fileResult) {
			res->writeStatus("404 Not Found")
			   ->writeHeader("Content-Type", "text/plain")
			   ->end("404 Not Found");
		...
		res->writeHeader("Content-Type", mimeType(filePath))
		   ->writeHeader("Cache-Control", "no-store")
		   ->end(std::string_view(reinterpret_cast<const char*>(fileResult->data()),
		                          fileResult->size()));
	});
...
void WsServer::stop()
{
	...
	// Capture raw Impl pointer (not `this`) to avoid use-after-free if the
	// WsServer is destroyed before the deferred lambda fires.
	auto* p = impl_.get();
	p->loop->defer([p]() {
		if (p->listenSocket) {
			us_listen_socket_close(0, p->listenSocket);
			p->listenSocket = nullptr;
		}
	});
}
...
void WsServer::broadcastSam2Video(std::vector<uint8_t> data)
{
	if (!impl_->loop) return;
	impl_->loop->defer([this, payload = std::move(data)]() {
		if (impl_->sam2VideoClients.empty()) return;
		std::string_view view(reinterpret_cast<const char*>(payload.data()),
		                      payload.size());
		for (auto* ws : impl_->sam2VideoClients) {
			ws->send(view, uWS::OpCode::BINARY, false);
		}
	});
}

void WsServer::broadcastScreenVideo(std::vector<uint8_t> data)
{
	if (!impl_->loop) return;
	impl_->loop->defer([this, payload = std::move(data)]() {
		if (impl_->screenVideoClients.empty()) return;
		std::string_view view(reinterpret_cast<const char*>(payload.data()),
		                      payload.size());
		for (auto* ws : impl_->screenVideoClients) {
			ws->send(view, uWS::OpCode::BINARY, false);
		}
	});
}

void WsServer::broadcastSecurityVideo(std::vector<uint8_t> data)
{
	if (!impl_->loop) return;
	impl_->loop->defer([this, payload = std::move(data)]() {
		if (impl_->securityVideoClients.empty()) return;
		std::string_view view(reinterpret_cast<const char*>(payload.data()),
		                      payload.size());
		for (auto* ws : impl_->securityVideoClients) {
			ws->send(view, uWS::OpCode::BINARY, false);
		}
	});
}

void WsServer::broadcastBeautyVideo(std::vector<uint8_t> data)
{
	if (!impl_->loop) return;
	impl_->loop->defer([this, payload = std::move(data)]() {
		if (impl_->beautyVideoClients.empty()) return;
		std::string_view view(reinterpret_cast<const char*>(payload.data()),
		                      payload.size());
		for (auto* ws : impl_->beautyVideoClients) {
			ws->send(view, uWS::OpCode::BINARY, false);
		}
	});
}

void WsServer::broadcastBlurVideo(std::vector<uint8_t> data)
{
	if (!impl_->loop) return;
	impl_->loop->defer([this, payload = std::move(data)]() {
		if (impl_->blurVideoClients.empty()) return;
		std::string_view view(reinterpret_cast<const char*>(payload.data()),
		                      payload.size());
		for (auto* ws : impl_->blurVideoClients) {
			ws->send(view, uWS::OpCode::BINARY, false);
		}
	});
}
