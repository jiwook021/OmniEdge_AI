...
# oe_find_inspireface()
#
# Find the InspireFace SDK (face recognition).
#
# Search order:
#   1. $ENV{INSPIREFACE_ROOT}/include + lib
#   2. System paths (/usr/local, /usr)
#   3. Local build output ($ENV{HOME}/omniedge_models/InspireFace/...)
#
# Exports to parent scope:
#   OE_INSPIREFACE_FOUND       — TRUE if header + lib found
#   OE_INSPIREFACE_INCLUDE_DIR — include path
#   OE_INSPIREFACE_LIB         — library path
# -----------------------------------------------------------------------------
function(oe_find_inspireface)
    if(DEFINED OE_INSPIREFACE_FOUND)
        return()
    endif()

    # Env-var override
    set(_iface_root "$ENV{INSPIREFACE_ROOT}")
    set(_iface_local "$ENV{HOME}/omniedge_models/InspireFace/build/oe-tensorrt-build/install/InspireFace")

    find_path(OE_INSPIREFACE_INCLUDE_DIR inspireface.h
        HINTS
            ${_iface_root}/include
            /usr/local/include
            /usr/include
            ${_iface_local}/include
    )
    find_library(OE_INSPIREFACE_LIB InspireFace
        HINTS
            ${_iface_root}/lib
            /usr/local/lib
            /usr/lib
    ...
    if(OE_INSPIREFACE_INCLUDE_DIR AND OE_INSPIREFACE_LIB)
        message(STATUS "[OmniEdge] Found InspireFace: ${OE_INSPIREFACE_LIB}")
        set(OE_INSPIREFACE_FOUND TRUE PARENT_SCOPE)
    else()
        message(STATUS "[OmniEdge] InspireFace not found (header: ${OE_INSPIREFACE_INCLUDE_DIR}, lib: ${OE_INSPIREFACE_LIB})")
        set(OE_INSPIREFACE_FOUND FALSE PARENT_SCOPE)
    endif()

    set(OE_INSPIREFACE_INCLUDE_DIR "${OE_INSPIREFACE_INCLUDE_DIR}" PARENT_SCOPE)
    set(OE_INSPIREFACE_LIB "${OE_INSPIREFACE_LIB}" PARENT_SCOPE)
endfunction()
...
function(oe_find_python_site_packages)
    ...
    set(OE_PYTHON_SITE_PACKAGES "${_site_packages}" CACHE PATH
        "Python user site-packages directory" FORCE)
...
function(oe_trtllm_ld_library_path OUT_VAR)
    ...
    set(LD_PATH "/usr/lib/wsl/lib:/usr/lib/x86_64-linux-gnu:${CUPTI_DIR}:${TORCH_DIR}:${NCCL_DIR}:${TRTLLM_DIR}:${CUDART12_DIR}:${ORT_ROOT}/lib:$ENV{HOME}/espeak_ng_dev/runtime/usr/lib/x86_64-linux-gnu")
...
function(oe_add_test TEST_NAME)
    ...
    if(OE_TEST_GPU_SPLIT)
        ...
        gtest_discover_tests(${TEST_NAME}
            TEST_FILTER "${OE_TEST_GPU_SPLIT}.*"
            PROPERTIES LABELS gpu RESOURCE_LOCK gpu_device
        )
    ...
    elseif(OE_TEST_LABELS)
        gtest_discover_tests(${TEST_NAME}
            PROPERTIES LABELS "${OE_TEST_LABELS}"
        )
    else()
        gtest_discover_tests(${TEST_NAME})
    endif()
