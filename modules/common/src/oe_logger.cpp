...
void OeLogger::initFile(std::string_view logDir, std::string_view fileName)
{
	...
	const std::string requestedPath = filePath.string();

	// Fast path: if we're already bound to the same file and it still exists,
	// keep existing sinks and only emit an attach marker.
	if (fileInitialized_ && !logOutputPath_.empty()
	    && logOutputPath_ == requestedPath
	...
	// stdout is an interactive terminal — when profile launchers launch modules
