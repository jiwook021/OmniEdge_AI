...
//
// Resolves model paths against the configured models_root. Absolute paths
// are returned as-is; relative paths are prepended with models_root.
//
// Usage in main.cpp:
//   const auto resolve = makeModelPathResolver(ini.paths().modelsRoot);
//   config.onnxModelPath = resolve(ini.tts().onnxModel);
// ---------------------------------------------------------------------------
...
///   - Otherwise the configured value (with env syntax stripped) is used.
...
makeModelPathResolver(const std::string& modelsRoot)
{
	...
	// Support "${OE_MODELS_DIR:-<default>}" shell-style syntax in config.
