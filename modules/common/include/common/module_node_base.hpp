...
//   1. initialize()  →  configureTransport() + loadInferencer() + publishModuleReady()
...
//   - The compiler enforces that every Derived implements configureTransport() and
//     loadInferencer().  A new module that forgets a hook is a compile error,
//     not a runtime bug.
...
public:
	...
	/// Calls Derived::configureTransport() → Derived::loadInferencer() →
	...
	void initialize()
	{
		...
		if (auto result = self.configureTransport(); !result) {
			throw std::runtime_error(
				"[" + std::string{self.moduleName()} + "] configureTransport failed: "
				+ result.error());
		}
		...
		if (auto result = self.loadInferencer(); !result) {
			throw std::runtime_error(
				"[" + std::string{self.moduleName()} + "] loadInferencer failed: "
				+ result.error());
		}
	...
	/// The atomic exchange ensures onBeforeStop() runs at most once even if
	/// stop() is called concurrently from multiple threads.
	void stop() noexcept
	{
