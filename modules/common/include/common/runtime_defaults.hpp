...
// overridden at runtime by config/omniedge.ini when the key is present
// (INI is the canonical config format; YAML is legacy).
//
...
//   - zmq/port_settings.hpp        — ZMQ port assignments
...
/// Set high enough to cover conversation-model load time (20-30s for Gemma-4 E4B).
...
/// Maximum time to wait for process reap after SIGKILL (s).
/// Prevents daemon hang if process is stuck in D-state (NFS, GPU driver).
inline constexpr int kKillReapTimeoutS = 5;
...
/// Graceful drain timeout when shutting down a module during a mode switch (ms).
/// Shared default for SwitchPlanConfig and [orchestrator_graph] INI section.
inline constexpr std::size_t kOrchestratorDrainTimeoutMs = 5000;
...
// ---------------------------------------------------------------------------
// LLM generation parameters (ConversationNode)
// ---------------------------------------------------------------------------
...
/// Minimum allowed maxTokens for GenerationParams::clamp().
inline constexpr int kGenerationMinTokens = 64;

/// Maximum allowed maxTokens for GenerationParams::clamp().
inline constexpr int kGenerationMaxTokens = 8192;
...
/// YOLOX-Nano (security) detection confidence floor.
...
/// Retry delay when a inferencer fails to load (ms). Used by loadInferencer() fallback loops.
