...
int runLifecycle(NodeT& node, std::string_view binaryName)
{
	try {
		...
		std::cerr << std::format("[{}] Initialize failed: {}\n",
		                         binaryName, e.what());
		// Exit 78 (EX_CONFIG): "dependency unavailable" — missing model file,
		// OpenCV without CUDA, SHM peer not ready after retries. The daemon
		// marks the module degraded and stops respawning. Restart would just
		// burn spawn cycles because the underlying config/artifact is broken.
		return 78;
	}
