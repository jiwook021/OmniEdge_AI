...
// each other. This replaces hard-wired direct calls between
// PipelineOrchestrator, VramGate, StateMachine, and the watchdog.
//
// Design:
...
/// (Gemma-4 E2B / E4B). The daemon kills the previous model process and
/// spawns the newly selected one.
struct ConversationModelChanged {
    std::string previousModel;     ///< e.g. "gemma_e4b"
    std::string newModel;          ///< e.g. "gemma_e2b"
    bool        needsTts{false};   ///< true if the new model requires a companion TTS process
};

/// Conversation model was auto-downgraded due to insufficient VRAM.
/// The daemon tried the user's requested model but fell back to a smaller one.
struct ConversationModelDegraded {
    std::string requestedModel;    ///< What the user asked for
    std::string actualModel;       ///< What actually loaded (smaller fallback)
    std::string reason;            ///< e.g. "vram_insufficient"
};

/// Conversation model was auto-upgraded back to the user's requested model.
/// VRAM was sufficient for 60+ seconds, so the daemon restored the larger model.
struct ConversationModelUpgraded {
    std::string fromModel;         ///< The degraded model that was running
    std::string toModel;           ///< The restored (larger) model
};
    ...
    void publish(const Event& event)
    {
        ...
        for (std::size_t i = 0; i < it->second.size(); ++i) {
            try {
                ...
                ++failureCount_;
                SPDLOG_ERROR("OE-COMMON-4001: EventBus handler threw (failures={}): {}",
                             failureCount_, ex.what());
            } catch (...) {
                ++failureCount_;
                SPDLOG_ERROR("OE-COMMON-4001: EventBus handler threw unknown exception "
                             "(failures={})",
                             failureCount_);
            }
        }
