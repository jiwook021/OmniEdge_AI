...
struct SessionHandle {
	...
	load(const std::string& modelPath, const SessionConfig& config)
	{
		...
		try {
			...
			const bool trtCacheCorrupt =
				what.find("plan.header.size") != std::string::npos ||
				what.find("deserializeCudaEngine") != std::string::npos ||
				what.find("TensorRT EP Failed to Build Engine") != std::string::npos;

			if (trtCacheCorrupt && config.useTRT) {
				const std::string cachePath = trtCachePath();
				OE_LOG_WARN(
					"onnx_trt_cache_corrupt: model='{}' error='{}' — purging '{}' and retrying once",
					modelPath, what, cachePath);
				std::error_code ec;
				...
				try {
					...
					return tl::unexpected(std::format(
						"Session creation failed for '{}' after TRT cache purge: {}",
						modelPath, ex2.what()));
				} catch (const std::runtime_error& ex2) {
					return tl::unexpected(std::format(
						"EP registration failed after TRT cache purge: {}", ex2.what()));
				}
			} else {
				return tl::unexpected(std::format(
					"Session creation failed for '{}': {}", modelPath, what));
			}
		}
