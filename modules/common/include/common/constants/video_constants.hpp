...
///
/// Why BGR24 over RGB or NV12:
///  1. OpenCV native — cv::imread(), cv::imdecode(), GStreamer videoconvert
///     all emit BGR24. Avoids a per-frame color swap at ingest.
///  2. GPU preprocessing — the fused CUDA kernel (resizeNormFP16Kernel) does
///     BGR→RGB as part of the normalize pass, so starting from BGR means one
///     fewer kernel launch vs starting from RGB and needing a separate swizzle.
///  3. Zero conversion at SHM boundary — every producer and consumer in the
///     video pipeline speaks BGR24, eliminating ~1-2 ms of CPU overhead per
///     1080p frame that an RGB conversion would add.
///  4. Industry convention — BGR is the de facto format for CV pipelines
///     (OpenCV, GStreamer, DirectShow).
