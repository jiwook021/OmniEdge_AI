...
tl::expected<float, std::string> SileroVad::classify(std::span<const float> pcm)
{
	...
	// ---- Input tensors ----
	// 1. audio input [1, chunkSamples] — use first 512 samples (pad if < 512)
	std::array<float, 512> inputBuf{};
	const std::size_t copyN = std::min<std::size_t>(pcm.size(), 512);
	...
	// ---- Extract outputs ----
	const float speechProb = *outputs[0].GetTensorMutableData<float>();

	OE_LOG_DEBUG("silero_vad_classify: prob={:.4f}, threshold={}",
	           speechProb, config_.speechThreshold);

	// Update GRU state for next chunk
	const float* hnData = outputs[1].GetTensorMutableData<float>();
	const float* cnData = outputs[2].GetTensorMutableData<float>();
