...
{
    OE_ZONE_SCOPED;
    OE_LOG_INFO("[SAM2] Loading encoder: {}", encoderPath);
    OE_LOG_INFO("[SAM2] Loading decoder: {}", decoderPath);
    ...
    OE_LOG_INFO("[SAM2] Model loaded successfully, VRAM: {} MiB",
                                 ...
                                 uint32_t       height)
{
    ...
    if (!impl_->modelLoaded) {
        return tl::unexpected(std::string("OE-SAM2-2001 SAM2 model not loaded"));
    }
    if (bgrFrame == nullptr) {
        return tl::unexpected(std::string("OE-SAM2-2002 null BGR frame pointer"));
    }

    OE_LOG_DEBUG("[SAM2] Encoding image {}x{}", width, height);
    ...
    OE_LOG_WARN("[SAM2] encodeImage: PLACEHOLDER — real ONNX inference not implemented");
    ...
    OE_LOG_DEBUG("[SAM2] Image encoded successfully");
    return {};
}
...
OnnxSam2Inferencer::segmentWithPrompt(const Sam2Prompt& prompt)
{
    ...
    if (!impl_->imageEncoded) {
        return tl::unexpected(std::string("OE-SAM2-2003 No image encoded — call encodeImage() first"));
    }

    OE_LOG_DEBUG("[SAM2] Running mask decoder with prompt type={}",
    ...
    case Sam2PromptType::kPoint: {
        if (prompt.points.empty()) {
            return tl::unexpected(std::string("OE-SAM2-2004 Empty point prompt"));
        }
    ...
    case Sam2PromptType::kMask: {
        if (prompt.mask.empty()) {
            return tl::unexpected(std::string("OE-SAM2-2005 Empty mask prompt"));
        }
    ...
    OE_LOG_DEBUG("[SAM2] Mask produced: {}x{}, IoU: {:.2f}",
                  result.maskWidth, result.maskHeight, result.iouScore);
                                  ...
                                  std::size_t       maxJpegBytes)
{
    ...
    // Step 3: Composite mask with chosen background mode as JPEG
    // In production, the GPU compositor applies blur/solid/color/overlay.
    ...
    if (simulatedJpegSize < 4) {
        return tl::unexpected(std::string("OE-SAM2-2006 output buffer too small for JPEG markers"));
    }
    ...
    // Fill body with a byte derived from the mask foreground count + bg mode.
    // This proves the pipeline produces output dependent on input, prompt, AND mode.
    ...
    OE_LOG_DEBUG("[SAM2] processFrame complete: {} bytes JPEG, mask fg: {}, bg_mode: {}",
                  simulatedJpegSize, fgCount, sam2BgModeName(bgMode_));
...
void OnnxSam2Inferencer::unload() noexcept {
    if (impl_) {
        ...
        OE_LOG_INFO("[SAM2] Model unloaded");
    }
}
