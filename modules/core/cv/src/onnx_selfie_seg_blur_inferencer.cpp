                                             ...
                                             uint32_t           inputHeight)
{
    ...
    // 0. Gate on OpenCV CUDA support. The pipeline uses cv::cuda::GpuMat
    //    buffers; if OpenCV was built without CUDA (as in the current NGC
    //    base image), every frame would throw throw_no_cuda from
    //    ensureFrameBuffers(). Fail fast here so the daemon reports a clean
    //    error instead of spamming warnings at 20 Hz.
    if (cv::cuda::getCudaEnabledDeviceCount() == 0) {
        throw std::runtime_error(
            "SelfieSeg: OpenCV built without CUDA support — bg_blur cannot run. "
            "Rebuild OpenCV with -DWITH_CUDA=ON or disable the background_blur module.");
    }
