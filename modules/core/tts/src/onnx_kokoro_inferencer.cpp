	...
	const std::string& voiceDir)
{
	...
	cudaOpts.arena_extend_strategy     = 1;     // kSameAsRequested — avoids large pre-allocation under VRAM pressure
...
void OnnxKokoroInferencer::unloadModel() noexcept
{
	...
	// Guard: only terminate if loadModel() was called (espeak_Initialize
	// must precede espeak_Terminate — calling Terminate without Initialize
	// is undefined behavior in the espeak-ng library).
	if (modelLoaded_) {
		espeak_Terminate();
	}
