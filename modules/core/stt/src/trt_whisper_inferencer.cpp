...
void TrtWhisperInferencer::loadVocabulary(const std::string& tokenizerDir)
{
	...
	for (const auto& [token, idJson] : vocabJson.items()) {
		const int tokenId = idJson.get<int>();
		if (tokenId >= maxId) maxId = tokenId + 1;
	}
	...
	if (auto addedText = readText(addedPath); addedText) {
		try {
			...
			OE_LOG_WARN("OE-STT-3001: added_tokens.json parse failed at {}: {} — "
			            "continuing with base vocabulary only",
			            addedPath.string(), ex.what());
		}
	}
	...
	for (const auto& [token, idJson] : vocabJson.items()) {
		const int tokenId = idJson.get<int>();
		if (tokenId >= 0 && tokenId < maxId) vocabulary_[tokenId] = token;
	}
	for (const auto& [token, idJson] : addedJson.items()) {
		const int tokenId = idJson.get<int>();
		if (tokenId >= 0 && tokenId < maxId) vocabulary_[tokenId] = token;
	...
	if (vocabulary_.empty()) {
		throw std::runtime_error(std::format(
			"OE-STT-5003: vocabulary is empty after loading from {} — "
			"Whisper cannot decode tokens",
			tokenizerDir));
	}
