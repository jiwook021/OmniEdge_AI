...
struct Sam2SegmentationStart {};
struct Sam2SegmentationStop {};
struct VisionModelStart {};
struct VisionModelStop {};
} // namespace fsm
...
using AnyEvent = std::variant<
    ...
    fsm::InterruptDone,
    fsm::Sam2SegmentationStart, fsm::Sam2SegmentationStop,
    fsm::VisionModelStart, fsm::VisionModelStop>;
...
[[nodiscard]] constexpr std::string_view eventName() noexcept {
    ...
    if constexpr (std::is_same_v<T, fsm::Sam2SegmentationStart>) return "sam2_segmentation_start";
    if constexpr (std::is_same_v<T, fsm::Sam2SegmentationStop>) return "sam2_segmentation_stop";
    if constexpr (std::is_same_v<T, fsm::VisionModelStart>) return "vision_model_start";
    if constexpr (std::is_same_v<T, fsm::VisionModelStop>) return "vision_model_stop";
