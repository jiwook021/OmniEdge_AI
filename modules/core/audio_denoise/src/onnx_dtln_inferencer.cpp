	...
	const std::string& model2Path)
{
	...
	// Load both stages via SessionHandle (CUDA EP only, arena capped to DTLN budget)
	// CUDA graph enabled: fixed input shape [1, 512] + fixed LSTM state [2, 1, 128]
	const oe::onnx::SessionConfig dtlnConfig{
		.useTRT         = false,
		.gpuMemLimitMiB = kDtlnMiB,
		.enableCudaGraph = true,
	};
	if (auto r = impl_->stft.load(model1Path, dtlnConfig); !r) {
		return tl::unexpected(std::format("OE-DENOISE-2001 DTLN stage1: {}", r.error()));
	}
	if (auto r = impl_->time.load(model2Path, dtlnConfig); !r) {
		return tl::unexpected(std::format("OE-DENOISE-2002 DTLN stage2: {}", r.error()));
	}

	// Pre-size LSTM hidden states to kStateSize and zero-fill.
	// Pre-sizing ensures stable data() pointers across frames,
	// which is required for CUDA graph replay.
	impl_->stftLstmState.resize(kStateSize);
	std::fill(impl_->stftLstmState.begin(), impl_->stftLstmState.end(), 0.0f);
	impl_->timeLstmState.resize(kStateSize);
...
{
	if (!impl_->stft.loaded) {
		return tl::unexpected(std::string("OE-DENOISE-2003 Model not loaded"));
	}
	...
	try {
		...
		if (stage1Outputs.empty()) {
			return tl::unexpected(std::string("OE-DENOISE-2004 DTLN stage 1 produced no outputs"));
		}
		...
		if (stage1Outputs.size() > 1) {
			...
			if (!newState) {
				return tl::unexpected(std::string("OE-DENOISE-2005 DTLN stage 1 LSTM state tensor data is null"));
			}
			...
			if (stateElements != kStateSize) {
				return tl::unexpected(std::format(
					"OE-DENOISE-2006 DTLN stage 1 LSTM state size mismatch: expected {}, got {}",
					kStateSize, stateElements));
			}
			// Use std::copy (not .assign) to preserve data() pointer stability
			// required for CUDA graph replay across frames.
			std::copy(newState, newState + stateElements, impl_->stftLstmState.begin());
		}

		// --- Stage 2: time domain refinement ---
		const float* stage1Out = stage1Outputs[0].GetTensorData<float>();
		if (!stage1Out) {
			return tl::unexpected(std::string("OE-DENOISE-2007 DTLN stage 1 output tensor data is null"));
		}
		...
		// Use std::copy to preserve data() pointer stability for CUDA graph.
		if (impl_->stage1OutputCopy.size() != stage1Elements) {
			impl_->stage1OutputCopy.resize(stage1Elements);
		}
		std::copy(stage1Out, stage1Out + stage1Elements, impl_->stage1OutputCopy.begin());
		...
		if (stage2Outputs.empty()) {
			return tl::unexpected(std::string("OE-DENOISE-2008 DTLN stage 2 produced no outputs"));
		}
		...
		if (stage2Outputs.size() > 1) {
			...
			if (!newState) {
				return tl::unexpected(std::string("OE-DENOISE-2009 DTLN stage 2 LSTM state tensor data is null"));
			}
			...
			if (stateElements != kStateSize) {
				return tl::unexpected(std::format(
					"OE-DENOISE-2010 DTLN stage 2 LSTM state size mismatch: expected {}, got {}",
					kStateSize, stateElements));
			}
			std::copy(newState, newState + stateElements, impl_->timeLstmState.begin());
		}

		// Extract denoised audio from stage 2 output
		const float* denoisedData = stage2Outputs[0].GetTensorData<float>();
		if (!denoisedData) {
			return tl::unexpected(std::string("OE-DENOISE-2011 DTLN stage 2 output tensor data is null"));
		}
		...
		return tl::unexpected(std::format(
			"OE-DENOISE-2012 DTLN inference error: {}", ex.what()));
	}
}
