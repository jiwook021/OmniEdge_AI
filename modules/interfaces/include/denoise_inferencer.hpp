...
public:
	...
	/** Denoise a temporal window of BGR24 frames, returning BGR24 instead of JPEG.
	 *
	 *  Same pipeline as processFrames() but skips the JPEG encode step.
	 *  Used in pipeline chaining mode where the output feeds into another
	 *  CV node via ShmCircularBuffer.
	 *
	 *  @param bgrFrames      Ordered list of BGR24 frame pointers (temporal window).
	 *  @param frameCount     Number of frames in the window.
	 *  @param width          Frame width in pixels.
	 *  @param height         Frame height in pixels.
	 *  @param outBgrBuf      Output buffer for BGR24 denoised frame.
	 *  @param maxBgrBytes    Size of outBgrBuf in bytes.
	 *  @return               Actual BGR24 byte count, or error string.
	 *
	 *  Default returns an error — concrete inferencers override when BGR24
	 *  output is supported.
	 */
	[[nodiscard]] virtual tl::expected<std::size_t, std::string> processFramesGetBgr(
		const uint8_t* const* bgrFrames,
		uint32_t frameCount,
		uint32_t width,
		uint32_t height,
	...
	{
		(void)bgrFrames; (void)frameCount; (void)width; (void)height;
